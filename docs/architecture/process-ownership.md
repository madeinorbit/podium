...
Status: agreed design (POD-2694), chosen by a unanimous three-reviewer blind
comparison over the lease-ledger alternative, which is kept for the record as
`process-ownership-lease-ledger.md` — its strongest mechanisms ride here as
fallback-lane witnesses. Implementation: POD-2691; the detailed plan is
`process-ownership-implementation.md`. Companion to the agent-runtime
architecture proposal (§9 phase 4) and `pod-2413-resource-isolation.md`.
