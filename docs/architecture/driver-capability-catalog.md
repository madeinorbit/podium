...
| **durable interrupt marker** survives provider reset and registry restart | pinned | pinned | pinned | wired | Headless drivers emit the synthetic `event: 'interrupt'` item; the server event log, session hydration, and transcript overlay retain it (`c6bd6e350`). Hermetic restart coverage is green; real-arm acceptance is still pending. |
...
| **promptForm** declared — both halves or unsupported | declared | declared | declared | declared | `path-text \| local-image \| file-part`, ONE form or one PER KIND: writing bytes no prompt can consume is not support (`capabilities.ts`); terminal declines when a raw first turn cannot keep path and text atomic; codex declares `image: local-image, file: path-text` because its `mention` part is accepted and then never reaches the model (POD-2819) |
...
| **cold start does not fake continuity** | pinned | pinned | pinned | n/a | `start()` emits a reset only for a newly created client generation; an adopted client keeps its scrollback. The discriminator is the spawn's `adopted` result, with regression coverage in `opencode-attach.test.ts` (POD-2761). |
| client parks and reconnects instead of cold-starting | absent | absent | wired | n/a | OpenCode declares `parkOnRelease` and reconnects its warm master; Codex/Grok remain deliberately non-parkable because their native clients own direct writers. |
...
| `parkable` / `revokeOnRelease` **declared** rather than branched on driver name | absent | absent | absent | absent | proposed in `attachment-lifecycle.md` §3.2; the current manifest has `parkOnRelease` for OpenCode, but the explicit `parkable`/`revokeOnRelease` capability pair and pure retention policy are still not modelled. |
