...
| LD8 | **PARTIAL** | P1 | §§2/6/8 require the Claude SDK embedded family in a worker child, selected for API-key/Bedrock/Vertex auth (`spec:170-184,503-533,551`). | The manifest and isolated persistent worker host now implement the `claude-sdk` path and its explicit subscription gate (`bcbbd6409`, `98ef8d6e0`); full product acceptance and matrix coverage remain outstanding. **Audit-time known/decided:** embedded/Claude-SDK rework was a plan non-goal; do not rewrite that historical classification. **Current policy, 2026-08-28:** that non-goal is superseded — Claude headless is first-class and subscription OAuth on the SDK is allowed. | **L**. |
