...
Reproduction: use the task-owned Bun 1.4.2 binary (or another verified 1.4.2 installation), materialize `git show 9bd1a5231:apps/server/src/sync/routes.ts` as `apps/server/src/sync/delta-baseline.measurement.ts`, and run `scripts/sync-measurements/delta-worker-run.py` from the repository root with `POD3953_BUN` set to that binary if necessary. It launches `delta-worker.mjs` with `--conditions=@podium/source` for each arm. Remove the temporary baseline route afterward. Raw measurements are attached to the issue.

## Validation and compatibility

Validation used the task-owned Bun 1.4.2 binary through sanctioned admission lanes. The strict native-worker differential passed for owner, granted reader, revoked reader and stranger, including a grant-only change without a head advance. Snapshot tests passed for identity, gzip and zstd, including concurrent append/prune, cursor refusal and an empty range (3 cases, 21 assertions). The focused server group passed 72 tests across 7 files; the sync kernel group passed 35 tests across 2 files.

The complete server-store shard was not green: 3,950 of 3,961 tests passed across 225 files, with 7 failing files and one unhandled error. Eight assertions across four files and the transcript-indexer SchedulerClosedError match POD-4014. Snapshot discovery timing is tracked separately as POD-4035 and passed on the focused run. The remaining two failures were test setup issues corrected here and verified in the focused lanes: real Worker tests moved to the existing native Bun lane, and an identity response header expectation corrected. This is focused repair evidence, not a claim that the entire shard passed.

The compiled smoke initially failed on Bun 1.4.2 (0 pass, 1 fail): Worker rejected a file URL string with ERR_WORKER_PATH, surfaced as SyncWorkerError unavailable. Converting only file:// targets to URL objects makes the standalone bootstrap-and-delta smoke pass (1 pass, 0 fail); Windows absolute embedded targets remain strings. POD-4037 records this compatibility fix. The coordinator independently confirmed the same integration tree passes on Bun 1.3.14 and fails on 1.4.2 before normalization. Other affected compiled callers were found in daemon discovery and janitor and filed separately as POD-4039.

The pending-promise matcher correction is isolated in commit 69329c656: only native test settlement and bounded test deadlines changed. No production scoping or worker producer behavior changed for that correction.

Final gate on integration `f99b25a80`: **lean gate green** (26 typecheck tasks, transaction-effect lint, 126 tests across the required 4 of 1,339 unit files). `bun run audit:serving-path` passed its detector probes and real-tree audit. This is not a full-suite result.
