...
| `bun run test:machine-updates` | Exact `scripts/machine-updates.integration.bun.test.ts`; independent Unix-socket supervisors, role processes and native helper | No parent command | Supervisor machine update ownership, authorization, staging, handover and recovery |
| `bun run test:native-machine-updates` | `scripts/native-machine-update.integration.ts`; real Linux Tauri/WebKitGTK, private signed debug ELF versions, HTTPS feed and zero-role parent; requires the headless staging produced by `test:multi-instance` and Rust/Xvfb/OpenSSL | No parent command | Native supervisor command transport, signature refusal, installation, shell death/restart and artifact confirmation; excludes packaged AppImage/FUSE, macOS and Windows |
