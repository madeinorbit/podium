...
| `packages/logger` | L0 model | neutral | logging | **landed POD-1900** (chunk 1 of the POD-1897 logging epic): the logging core every runtime shares — `createLogger(ns)`, five levels, child-bound context, the NDJSON record shape, a fail-open sink fan-out, and the console and ring-buffer sinks. The second L0 leaf, and it has to be one: every layer logs, so a logger anywhere higher would be an upward import from somewhere. Dependency-free (not even zod). Retagged `browser-safe` → `neutral` by POD-1901 (chunk 2), which added the Node-only file and stdout sinks behind a `./node` subpath: the barrel stays browser-safe and stays the declared browser entrypoint, and the neutral tag is what makes `manifest-browser-reach` refuse a browser-safe workspace reaching past it into `./node` |
...
| `packages/commands` | L1 wire | browser-safe | command-contracts | **landed POD-728** (3.9b) — created ahead of POD-311, which migrated the issue registry onto the L1/L3 split, folded in the live `protocol/commands.ts` framework, and derived transports from `exposure`. The speculative generic mutation envelope was deleted; durable production writes use `OutboxCommand` from `packages/sync`. |
...
| `packages/agent-runtime` | L2 kernel | node-only | agent-runtime-contract | **landed POD-2019** (POD-1761 W1): the Agent Runtime contract — the typed primitive surface every harness driver sits behind (lifecycle, turns, interactions, observation, transcript, attach, export), its zod wire projection in `@podium/protocol`'s `runtime` family, and the driver-conformance corpus. A PORT, not a feature: it owns no engine and composes the two ports below it. Takes the host capability (its drivers spawn abduco masters, harness servers and SDK worker children) with `@podium/agent-runtime/metadata` as the declared open surface for `apps/server` and `@podium/agent-runtime/testing` shipping the corpus W3/W5/W6 run against their own drivers. Principal-free (`manifest-principal-free` lint) |
...
`pty → runtime` (POD-396: `stateDir()` behind the abduco binary cache); `harness → runtime`; `harness → transcript`; `agent-runtime → harness` (POD-2019: the driver taxonomy and the three `*RuntimeSpec` shapes are DEFINED in harness beside the manifest axis that declares them and re-exported by agent-runtime — the reverse direction would be a cycle; `agent-runtime → pty` is deliberately deferred — W3's terminal driver put its concrete half in `apps/daemon`, so the PTY import stayed there, and W5's opencode driver keeps its process management there too); `agent-runtime → transcript` (POD-2023 W5: the opencode server driver's SSE payloads are the SAME message+part pair the sqlite transcript source already reads, so it calls `opencodePartToItems`/`stampOpencodeItems` rather than shipping a second opencode→TranscriptItem mapper — two mappers for one harness is two renderings of the same tool call, diverging on the first tool opencode adds; port → parser, both L2, no capability crosses); `terminal-client → composer`; `terminal-client-react → client-core` (POD-400: React hooks bind the client-core socket-transport port); `commands → protocol` (POD-728: contracts name the frames they are exposed on); `issue-client → commands` (POD-311: the CLI's rendering table RENDERS the shared contracts rather than declaring its own command-name universe — an edge that existed since POD-311 and was invisible to this list until POD-335 stopped exempting type-only imports from the layer axiom).
...
**Field classification.** `readAt`, snooze, pins, tab order and sidebar/tab layout are settled per-user state and every session-scoped read/write resolves the calling principal's HUMAN `userId`; an agent carries its actor session separately and writes the on-behalf-of human's row. Sidebar/tab layout lives in `user_layout` (POD-1350) behind `layout.set` / `layout.get`; device-local route, selection, focus and pane/split geometry stay in client ui-state (POD-403). Personal preferences live behind the principal-scoped settings module and `user_preferences` store. Reaching into either sibling would share protected state merely to co-locate a taxonomy, so the modules share the `(userId, entityId)` contract instead.
