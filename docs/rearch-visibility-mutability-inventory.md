...
1. **47 of 83 classes have mutable visibility.** This is the majority of
...
| `pending-interactions` | Pending interactions | personal | `share` `unshare` `revoke` | PHASE 2 MUST HANDLE: follows its session. |
...
| `feed-read-cursor` | Event-stream read cursor | per-user-state | — | Non-grantable by construction, so no verb can change who sees it. The only lifecycle event is the owner’s account being removed. |
...
| `managed-credentials` | Managed credentials / connected accounts (`managed_credentials`) | secret | — | Values never replicate; `manage` is admin-grade. |
