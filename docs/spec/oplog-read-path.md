...
- **Retention** *(corrected post-implementation — see [ADR 2](../adr/0002-sync-protocol.md) D5, which is authoritative)*: prune from the head by count/age. The two budgets are **bounds on the log**, and they are reconciled as *whichever deletes more* — `thresholdSeq = max(rowCapSeq, agedSeq)` (`sync-repository.ts` `planChangePrune`), which honors both bounds. Defaults: **20 000 rows** and **3 days** (`CHANGE_KEEP_ROWS` / `CHANGE_MAX_AGE_MS`, `packages/sync/src/change-log.ts`). The original draft of this spec said "20 000 rows or 14 days, whichever is **larger**"; that is wrong twice over — it never matched the shipped number, and "whichever is larger" honors *neither* bound. `minAvailableSeq` = lowest retained seq, and it is a **published protocol parameter**, not an implementation constant, so a replica can tell it needs a re-bootstrap before asking (code-side work: POD-305). Pruning only from the head keeps the retained range contiguous; catch-up replays are idempotent upserts, so redundancy is harmless. Retention is a **liveness/cost knob** — it tunes how often clients re-bootstrap, and by ADR 2 D5's argument it cannot cause divergence: a replica past the horizon is served a snapshot, which is positive state. That property depends entirely on pruning staying head-only.
- **The snapshot must not live in the pruned table** *(POD-678, corrected post-implementation)*. The paragraph above is only sound while "a replica past the horizon is served a snapshot" is true, and it was not: the snapshot was a fold over `changes`, so retention deleted the fallback along with the window. On the live install the 20 000-row budget bit after ~27 hours and a bootstrap carried 66 of 631 issues — every `POD-N` mention of the rest rendered as unknown. The world therefore lives in its own table (`change_latest`: latest live upsert per (entity, id), written by the append, never pruned), and the row budget now bounds only what `changesSince` can serve. Retention is a knob on the WINDOW; there is no budget on what exists.
