...
| **Pending member invite** (`member_invites`) | the inviting user (`created_by`); the invitee has no identity until claim | `personal`; no sync projection. The inviter may read their own outstanding metadata without admin rights, while active admins may manage all invites. The bearer token remains `secret-value`. The email carried before an account exists is an O1 existence fact | none; invite management is a membership command, not a grant |
...
naming a grantee requires disclosing that the grantee exists. The pending-invite row makes the same open question concrete one step earlier: its invited email is an existence-bearing directory fact before an account exists. It is marked, not resolved.
...
|---|---|---|---|---|
| **O1** | **Which existence facts leak** — counts, machine session lists, "this worktree is in use", lock holders, issue ref-letter allocation, cross-owner blob dedup, and an invite's email before its account exists (readiness §3.1.2) | Whether an existence-bearing row is `deployment-substrate` or must be suppressed from a scoped slice; today's matrix marks the sites (§3 blobs, §7 locks, §11 pending invites) without resolving them | Feature owner per surface, against ADR 9's visibility classes | Phase 3 policy (POD-290). The consistent-error rule for one site is already fixed (readiness §3.1.5) |
