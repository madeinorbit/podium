...
| 2 | `IssueWire` | `packages/protocol/src/messages/issues.ts` | wire/read (71 keys counted at the pre-POD-797 tip). Now the **session-free** transitional wire residue for capless clients and hub mirrors; expiry POD-309/POD-827. |
...
| Composed from shared field schemas | **Done** at POD-643 (`41d5ab0d`), against POD-365's landed groups. Every session and issue field is the shared schema INSTANCE; three are `.unwrap()`ed because an export is a checkpoint and a live session may lack what a bundle must have. Asserted by reference identity, not by shape — a fresh `z.string()` restatement is byte-identical on the wire and reds only that one assertion. **This decision is not a composition exemption**, and D2's rejected-alternatives row says so explicitly ("treat HandoffManifest as a protocol-only one-off" is refused): D4 retains the *representation* and requires its *fields* to compose. The permanent thing is the declaration site, not any field debt. |
| Attribution pair + owner | **Deferred to POD-1142** with the rationale recorded: the pair is structurally unsplittable (timestamp nested inside the actor object), and on a FILE format that cannot arrive additively — it needs `format: 2` with a v1 arm, and the v1→v2 read path is POD-644's transfer path. |
...
entity (D7.4). `IssueWire`'s former embedded `SessionMeta[]` was the canonical
non-compliance; the Issues pilot **deleted it in POD-797**. The retained session-free
`IssueWire` residue is registered with expiry at POD-309 or POD-827, whichever lands
first.
