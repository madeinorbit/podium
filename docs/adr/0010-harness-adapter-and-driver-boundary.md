...
- Nothing inside `adapters/` **MUST** import a mechanism. Nothing in a mechanism **MUST** import a specific adapter, only the Adapter type. **Amended 2026-09-22 (3.R D6):** a driver family whose directory is named after a harness (`driver/families/codex`, `opencode`, `grok-acp`, `claude-sdk`) **MAY** import its own adapter directory and no other; the terminal family, `driver/host.ts`, `driver/runtime.ts`, `store/`, `inventory/` and `observer.ts` **MUST NOT** import any specific adapter. Unchanged: the Store's transcript sources stay the one reader of transcript grammars (a rule-2 narrow reader), and the registry stays the one composition root. An import check enforces it.
