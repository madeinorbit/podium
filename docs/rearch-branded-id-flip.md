...
|---|---|---|
| `apps/server` store + projections (`store/automations.ts`, `modules/issues/service/{core,reads,crud}.ts`, `modules/sessions/{session,service}.ts`, `modules/memory/service.ts`, `repo-registry.ts`) | 8 | A sqlite row / the server's own runtime object types its ids as plain strings. **Replace by branding the row and runtime types** (`IssueRow.id`, `Session.sessionId`, …) — one change that removes most of this column. POD-361 did not, because branding `IssueRow` alone ripples through every write site: that is POD-362's sweep, not a projection cast. |
