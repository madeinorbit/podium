...
| SQLite on Bun | `@podium/runtime/sqlite` shim (node:sqlite ⇄ bun:sqlite); shared spec green on both; full `SessionStore` round-trip on bun:sqlite. (Historical: the node:sqlite driver was removed in PDM-25 — the shim is bun:sqlite only.) |
