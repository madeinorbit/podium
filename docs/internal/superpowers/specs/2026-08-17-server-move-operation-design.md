...
onAction?(input: {
  ...
  mode: 'engine' | 'sealed',   // sealed = sealed row or narrate/recovery boot; no engine queue, no table writes
...
- **Routing, defined precisely at the operations router/facade — this is what makes sealed actions deadlock-free.** For an **unsealed** row, `settleAsk`/`action` queue through the engine as today (`mode:'engine'`; the engine persists the settle). For a **sealed** row (and equally in narrate/recovery-only boot, §5.4), the row can never re-enter the engine: `engine.settleAsk` refuses post-seal (§2.1), and on a live sealed source the sealing runner's `ensure()` may still be unresolved and **owns the engine's operation queue** — a queued dispatch could never enter. So the router does not queue: it validates the requested `actionId` against the **deterministic projected** ask/action (§6.1) computed from (row bytes, journal), and on match calls the kind handler **directly, in `mode:'sealed'`, outside the engine write queue**, with **zero operations-table writes**. The handler may mutate **only journal and config facts** — file-level writes legal under a fenced SQLite; the next projection read reflects them, and that *is* the state change. An `actionId` the projection does not currently offer refuses.
...
Tests: dispatch reaches the kind handler in both modes; sealed mode performs zero operations-table writes (query-only store proves it); **concurrency: with the sealing runner's `ensure()` deliberately blocked and unresolved (holding the engine queue), a sealed-mode dispatch validates against the projection, runs the handler to completion, and its journal fact is visible on the next projection read** — pinned so the sealed-action deadlock can never regress; actionId not offered by the current projection refuses; unknown actionId refuses; non-admin caller refused at the contract layer before the handler; handler result is returned to the caller verbatim; idempotent double-dispatch.
...
8. Cutover choreography, still this runner, all file-level writes legal under a fenced SQLite: promote → `healthProofMatches` (now including port, §8) → **`demoteSource` (config write, fsync) → `journal.commit` (fsync + dir-fsync)** → *only then* `acknowledge` to the target → `afterCommitted` → `retireSourceAfterTransfer`. Demotion and the committed journal are durable **before** the target is told to finalize and before any retirement step. **On any post-`committing` uncertainty** — lost promote reply, ambiguous proof, or a demote/journal-commit write failure after a successful promote — the runner records journal `commit-uncertain` where needed (`committing → commit-uncertain` is legal), returns `{state:'handed-off'}`, and **never resumes**: it does not retry, park, or wait, and no engine drive ever settles anything (SQLite is query-only and the row is sealed). The generic recovery action (§6.2) owns **all** subsequent fact reconciliation. Acknowledge failure after a durable commit is non-fatal (`result:'pending'` cleanup, as today).
...
- `journal.state === 'source-fenced'` → overlay: `fence` step `done` (the journal is the fact the row could not record), `cutover` `pending`, `detail:'ready to switch'`, plus — when no in-flight drive owns the row (the sealing runner has returned, or this is a recovery boot) — the recovery ask, since no runner will ever finish the cutover.
- `'committing'` → `fence` `done`, `cutover` `running`, `detail:'switching servers'`; with no in-flight drive, the recovery ask.
...
The handler runs under the **calling operator's live principal** (§7.2), always in `mode:'sealed'` for this kind's post-fence postures, and mutates **only journal and config facts** — never the operations table:

- Journal `source-fenced` (provably no promote, §4): operator may resolve **abort** — target stage aborted, fence released at the file level, journal `aborted` — and the handler then **triggers a supervised process restart**. The next **writable boot adoption** clears the stale seal (`details._handoff`) and records the terminal `failed`/aborted history row through the normal §5.5 accounting. A rebooted narrate/sealed handler **never invokes same-process `reclaimHandoff`** — reclaim remains strictly a sealing-runner, same-`ensure()` facility (§2.2); recovery gets to the same end state through journal facts plus boot adoption. Resolve **proceed** is refused (a dead drive cannot be resumed post-fence; the safe forward path is target-side).
- `committing | commit-uncertain`: run `inspectUncertain` (`service.ts:673-722`), **convergent even when the promote request never reached the target**:
  - Target promoted with byte-exact serving proof (incl. port) → `demoteSource` → `journal.resolveCommitted` → acknowledge → outcome `resolved-committed`.
  - Target reports the **exact matching validated/staged transfer** — identity byte-match on all of transferId, manifest digest, publicUrl, port — but not promoted (the promote request was dropped in flight): the handler **reissues the byte-identical promote** (promote is full-object idempotent replay, §3.2), verifies the serving proof, then demote → commit → acknowledge → `resolved-committed`. `committing` was durable before the first send attempt (§4 step 7), so reissue is always legal here.
  - Target proof/identity mismatched, target unreachable, or state unknowable → outcome `still-uncertain` with operator guidance; the journal deliberately refuses `commit-uncertain → aborted` on proof-absence alone. Idempotent, re-runnable after restarts.

Reaching the uncertain posture without a crash (e.g. lost promote reply) is not a special case: the runner already recorded `commit-uncertain` and returned `handed-off` (§4 step 8) — nothing parks, retries, or resumes. The web renders one button — "Check the new server" — that calls `operations.settleAsk` (sealed dispatch, §2.3). `checkTarget` and re-mutate recovery are deleted.

Focused tests: dropped promote **request** (target still staged/validated, byte-identical reissue converges to committed); dropped promote **reply** (target already promoted, proof path converges without reissue); mismatched identity stays `still-uncertain`; double dispatch of either converged path is idempotent; source-fenced abort → restart → boot adoption clears the seal and lands the terminal row.
...
- **Once `committing` is durable or a promote may have been sent**, the stored actor's authorization no longer gates anything: what remains is fact reconciliation, not a privileged act on the old actor's behalf. In this region no drive resumes and nothing parks (§4 step 8): the machine simply presents the §6 posture, and finishing requires a **currently authorized operator** invoking the generic recovery action (§6.2), authorized by their own live principal.
...
Tests: phase-boundary matrix (each phase × {actor ok, actor revoked, actor missing, intent mismatch}); resumed-drive reauthorization uses stored identity with zero request context; commit-boundary revoked pre-promote → reclaim (writable, unfenced, `failed`, no promote ever observed by the target harness); post-`committing` revoked → §6 posture with no drive activity, resolvable only via the recovery action under a live admin principal; recovery action refuses non-admin callers; adoption never consults authz.
...
4. **Lost promote message, full recovery — both directions**: (a) control-proxy drops the promote **reply** (target promoted, source unsure); (b) control-proxy drops the promote **request** (target still staged/validated, never promoted). In both, the source records `commit-uncertain`, returns handed-off, and presents the uncertain waiting posture with the `server-move-recovery` ask via the projection; drive recovery **only** via `operations.settleAsk` (the old re-mutate path and any kind-named RPC must not exist); in (b) assert the handler reissued the byte-identical promote (§6.2); in both, assert convergence to `done` on the target and demoted source.
