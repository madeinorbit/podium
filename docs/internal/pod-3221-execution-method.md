...
| Threat | Answer | Cost |
|---|---|---|
| A repository is left half-converted, or raw SQL survives on the drizzle type | One lint rule family over the store directories: no runtime-sqlite import, no `.prepare(`, no whole raw statement on drizzle's raw-execution methods except rule 1's marked `UPDATE OR <conflict-algorithm>` builder gap, no `PRAGMA`, `sqlite_master` or `ATTACH` inside `sql` bodies, no drizzle transaction outside the store's port, drizzle imported only from the store, the operations store, the migrations and the sync adapter. Its only unanswered-site allowlist is the `// DECISION POD-<n>` marker; the permanent UPDATE-conflict token is accepted only when the statement shape also matches. Deleting the executor's legacy handle field at Stage A exit is a free second check. | One permanent rule family, proven to fire on both sides of each exception. |
