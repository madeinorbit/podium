...
Subject: **Grok Build CLI** — xAI's interactive coding-agent TUI, binary `grok` (verified here as `grok 0.2.101 (5bc4b5dfad) [stable]` at `~/.local/bin/grok`). Podium uses its interactive PTY surface, native personal hooks, persisted sessions, and shared issue relay to run Grok as a first-class intra-issue implementation child. Headless `grok -p` and ACP `grok agent stdio` remain available for non-interactive consumers.

Citation conventions used below: **[help]** = the installed `grok --help` / subcommand help; **[on-disk]** = files under `~/.grok`; **[official docs]** = the current SpaceXAI Grok Build documentation.

---

## 1. Discovery & identity

- **Binary detection / "installed?":** binary name `grok` on PATH. Decide installed by PATH lookup plus a successful `grok --version` (also `grok version`), which reports semver, git hash, and channel; the verified binary reports `grok 0.2.101 (5bc4b5dfad) [stable]`. Process recognition and terminal labeling use the exact `grok` executable identity. *(established)*
- **Version detection / gating:** `grok --version` / `grok version`; the channel is included. Hook and model behavior in this integration is verified against 0.2.101, while the file observer remains a compatibility fallback for older installs. *(established)*
...
- **Option-like prompts need `--` (measured, POD-1317):** grok's clap parser rejects a positional starting with `-` — `grok "- remove …"` exits 2 with `error: unexpected argument '- ' found`. Use `grok [OPTIONS] -- "<prompt>"`; nothing may follow the prompt.
- **First-run / trust handling — two surfaces:**
  - **Project trust (for hooks/plugins):** projects must be explicitly trusted before project `.grok/hooks/` run. Podium does not require that prompt: it installs its env-gated hook in the personal `$GROK_HOME/hooks/podium.json` scope, which needs no project trust. `grok inspect --json` confirms discovery.
...
- **Model / reasoning effort at launch:** `-m/--model <MODEL_ID>` and `--effort <low|medium|high|xhigh|max>`. The authenticated 0.2.101 catalog exposes exactly `grok-4.5` (default) and `grok-composer-2.5-fast` (Composer 2.5 Fast); Podium forwards either exact ID unchanged. *(established)*
...
- **Podium intra-issue mode [spec:SP-79c5]:** coordinators spawn `podium agent spawn --issue <id> --harness grok --model <id> --prompt <task>`. The adapter appends the issue/spec/workflow contract through `--rules`; every child receives `PODIUM_SESSION_ID` and `PODIUM_ISSUE_RELAY`, so `podium issue prime`, workflow checkpoints, and durable `podium issue mail` / `podium mail reply` commands work without harness-specific setup.
...
- **Model listing for a settings UI:** `grok models` is the authoritative live probe used by Podium; it lists IDs and marks the default. On the authenticated 0.2.101 install it returns `grok-4.5` (default) and `grok-composer-2.5-fast`. When probing is unavailable, the web picker uses those same two IDs as its static fallback. *(established)*
...
- **Usage windows / quota / rate-limits:** Grok exposes both the monthly credit ledger and the shared weekly pool through the CLI proxy with the OIDC token from `~/.grok/auth.json`: `GET https://cli-chat-proxy.grok.com/v1/billing` returns `config.monthlyLimit.val` / `config.used.val` / `billingPeriodEnd`, while `GET https://cli-chat-proxy.grok.com/v1/billing?format=credits` returns `config.creditUsagePercent` and the weekly `config.currentPeriod.start/end`. Podium's daemon merges both windows into the top-bar quota popup. Public `api.x.ai` has no remaining-quota endpoint (RPS/TPM tiers only). *(established for the Build billing endpoints)*
...
- **Signal sources, by authority:**
  ...
  3. **Native hook callbacks** (see §8): camelCase `hookEventName` payloads POST to the per-session daemon endpoint. Podium maps prompt/tool/notification events to **working**, AskUserQuestion and PermissionDenied to **needs user**, Stop/SessionEnd to **idle/ended**, StopFailure to **errored**, compaction to its explicit phase, and SubagentStart/SubagentStop to task deltas.
...
- **Managed install [spec:SP-79c5]:** production daemons idempotently maintain `$GROK_HOME/hooks/podium.json`. Personal hooks need no project trust. Foreign groups in the dedicated file are preserved; corrupt content is left untouched; hosts without a Grok home are skipped.
- **Per-session binding:** every handler is the same env-gated command. Podium-spawned sessions inherit `PODIUM_GROK_HOOK_URL`; other Grok runs exit successfully before any network access. The command POSTs stdin JSON to the loopback ingest with a two-second curl bound and fails open.
- **Native payload and lifecycle:** Grok sends camelCase `hookEventName`, `sessionId`, `cwd`, `workspaceRoot`, and tool fields. Podium registers SessionStart/End, UserPromptSubmit, Pre/PostToolUse, PostToolUseFailure, PermissionDenied, Notification, Stop/StopFailure, SubagentStart/Stop, and Pre/PostCompact.
- **Blocking contract:** only PreToolUse consumes stdout decisions, using `{"decision":"deny","reason":"…"}`; passive Stop output is ignored by Grok. Durable unread mail and acknowledgement reminders therefore deny one PreToolUse call with the Podium inbox/reply pointer, then the cooldown allows the agent's retry. Stop remains an authoritative completion signal.
- **Fallback and verification:** the existing `updates.jsonl` observer remains active if hooks are unavailable. `grok inspect --json` in an isolated home verifies the managed file is discovered, and the hook transport has an executable callback test.
...
- **Cost/latency/timeout/output caps:** `--max-turns` caps turns; **no built-in wall-clock timeout flag** (wrap with your own); `streaming-json` lets you stream/abort early. The verified default is `grok-4.5`; `grok-composer-2.5-fast` is the faster Composer option. No per-call cost output (§5).
...
| Hooks | **Yes** — native personal `$GROK_HOME/hooks/podium.json`, env-gated per Podium session; project trust is not required; file observation remains the fallback |
...
| Usage / quota | Account credit quota via `cli-chat-proxy.grok.com/v1/billing` plus the weekly pool via `?format=credits`; **rich session token/latency accounting** in `signals.json` |
| Pricing / cost | **No** — compute externally |
| Fast model | **Yes** — `grok-composer-2.5-fast`; primary/default model `grok-4.5`; reasoning effort is separately selectable |
...
- **Agent-specific limitations:** `agent serve` binds `127.0.0.1:2419` by default (localhost — bind/tunnel deliberately for remote). Local-only files (`auth.json`, `leader.sock`, sessions) live under `GROK_HOME` on whichever host runs `grok`; multi-account remote = per-account `GROK_HOME`. Account credit quota is via the Build billing HTTP endpoint (§5), not a local file.
