...
- **Usage windows / quota / rate-limits:** Account **credit** quota is readable via `GET https://cli-chat-proxy.grok.com/v1/billing` with the OIDC token from `~/.grok/auth.json` (same surface as TUI `/usage` / `/cost`). Response is a monthly credit pool (`config.monthlyLimit.val`, `config.used.val`, `billingPeriodEnd`) — not Claude-style 5h/weekly windows. SuperGrok multi-product weekly pool (Settings → Usage on grok.com) is a separate surface and not available to the Build OIDC token. Public `api.x.ai` has no remaining-quota endpoint (RPS/TPM tiers only). Podium's daemon maps the monthly pool into the top-bar quota popup. *(established for Build billing endpoint; SuperGrok weekly pool separate)*
...
| Usage / quota | Account credit quota via `cli-chat-proxy.grok.com/v1/billing` (monthly); **rich session token/latency accounting** in `signals.json` |
...
- SuperGrok multi-product weekly usage pool (Settings → Usage) — machine-readable path for Build OIDC still open; Build monthly credits use `cli-chat-proxy.grok.com/v1/billing`.
