/**
 * Real production-driver A1b proof for POD-2920.
 *
...
type DriverName = 'codex' | 'opencode'
type LedgerRow = {
  id: string
  body: string
  status: string
...
if (driver !== 'codex' && driver !== 'opencode') {
  throw new Error('usage: a1b-runtime.ts codex|opencode')
}

const evidenceDir = process.env.POD2920_EVIDENCE ?? import.meta.dir
const base = process.env.PODIUM_DRIVE_BASE ?? '/tmp/pod-2920-qpos'
const cwd = resolve(base, 'probes', `${driver}-a1b`)
const expectedDriver = driver === 'codex' ? 'codex-app-server' : 'opencode-server'
...
function verifyPins() {
  const result = spawnSync('bash', [resolve(evidenceDir, 'verify.sh')], {
    cwd,
    env: process.env,
    encoding: 'utf8',
  })
...
try {
  ...
  const controlToken = nonce(`P2920-${driver.toUpperCase()}-CONTROL`)
  ...
  const replyToken = nonce(`P2920-${driver.toUpperCase()}-QUEUED-REPLY`)
...
const output = resolve(evidenceDir, 'readings', `${driver}-a1b.json`)
