...
#
# A ROW LEAVES THIS LIST THE MOMENT ITS CELL IS DRIVEN.
#
# A10 half 2 was here, correctly flagged RE-CHECK because POD-2853 had landed.
# It was then driven and PASSED — and the row stayed, so every subsequent run
# shouted RE-CHECK about a cell that was finished. A checker that always reports
# something to do is a checker people stop reading, which is precisely the
# failure it exists to prevent. Prune on drive, not eventually.
#
