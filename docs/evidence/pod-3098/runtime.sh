...
RELAY_PORT=46984
PASSWORD=p3098-a3
BASE_PIN=b64abaafd7f3c4dcaa034464e98d7304d1f175b3
PIN=ef1b5e4d421e62e4e93e3fcec151eff75b832bbc
...
verify_runtime() {
  ...
  printf '%s' "$stamp" | grep -Eq '"sourceSha"[[:space:]]*:[[:space:]]*"ef1b5e4"' || fail "served web pin is not ef1b5e4: $stamp"
  echo "PIN VERIFIED head=$PIN server=$(sed -n '1p' "$BASE/server.sha") daemon=$(sed -n '1p' "$BASE/daemon.sha") web=ef1b5e4 arm=$arm instance=$INSTANCE state=$STATE_ROOT agentHome=$AGENT_HOME ports=$PORT/$HOOK_PORT/$RELAY_PORT"
}
