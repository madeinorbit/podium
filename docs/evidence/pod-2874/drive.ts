/**
 * POD-2874 — Tier-A release acceptance for Claude and plain shell.
 *
...
async function runA1b() {
  ...
  try {
    ...
    const position = (payload as Record<string, unknown> | null)?.position ?? (payload as Record<string, unknown> | null)?.queuePosition ?? null
    const hasPositionField = typeof position === 'number' || /\b(?:queuePosition|position)\b["']?\s*:/i.test(text)
...
async function runA1c() {
  ...
  try {
    ...
    const control: Control = { fired: base.user.ok, what: 'a baseline prompt appearing as a durable user turn before death', detail: 'baseline user=' + base.user.ok + '; baseline reply=' + base.assistant.ok }
    ...
    await wait(1_000)
    const dead = 'P2874-A1C-DEAD-' + Date.now().toString(36).toUpperCase()
    ...
    return result(typed && !delivered ? 'PASS' : 'FAIL', typed && !delivered ? 'dead-session send returned a typed refusal and did not disappear silently' : 'dead-session send was accepted without a typed refusal', control, ['BASELINE          ' + short(base), 'DEAD SEND         ' + short(payload), 'TYPED REFUSAL     ' + typed, 'SILENT ACCEPT     ' + delivered], { sid, dead, payload, typed, delivered })
  } finally { await cleanup(sid, chat) }
}
...
async function runA5() {
  const { sid, chat } = await create('claude')
  try {
    const secret = 'P2874-A5-' + Date.now().toString(36).toUpperCase()
    writeFileSync(join(cwd, 'transcript-fixture.txt'), 'transcript fixture secret ' + secret + '\n')
    const sent = await mutate('sessions.sendText', { sessionId: sid, text: 'Use your Bash tool to run cat ' + join(cwd, 'transcript-fixture.txt') + ' and then reply with only the secret word it contains.' })
    const user = await waitForNeedle(sid, chat, 'transcript-fixture.txt', 'user', 30_000)
    const assistant = await waitForNeedle(sid, chat, secret, 'assistant', REPLY_MS)
...
async function runA8() {
  ...
  try {
    ...
    if (!sid) throw new Error('sessions.create failed ' + short(made))
    chat = new Chat(sid); await chat.open(); await wait(8_000)
...
function sessionProcesses(target: string): string[] {
  const ps = outputOf('ps', ['-eo', 'pid=,ppid=,args='])
  return ps.split('\n').map((line) => line.trim()).filter(Boolean).filter((line) => {
    const fields = line.split(/\s+/)
    const pid = fields[0]
    if (!/^\d+$/.test(pid) || Number(pid) === process.pid) return false
    const processCwd = outputOf('readlink', [join('/proc', pid, 'cwd')])
    return processCwd === target || line.includes(target)
  })
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    switch (cell) {
      ...
      case 'A4A': out = await runA4a(); break
      case 'A4B': out = await runA4b(); break
