...
run_down() {
  if [ "${P3152_ATOMIC_TEST_MODE:-0}" = 1 ]; then
    "$P3152_ATOMIC_TEST_RIG" down
  else
    bash "$RIG" down
  fi
}
...
require_live_pid() {
  ...
  tr '\0' '\n' <"/proc/$pid/environ" | grep -Fx "PODIUM_INSTANCE=$P3152_INSTANCE" >/dev/null || {
...
real_run() {
  : "${P3152_RUN_TOKEN:?P3152_RUN_TOKEN is required}"
  # shellcheck source=docs/evidence/pod-3152/rig-env.sh
  source "$HERE/rig-env.sh"
  [ "${P3152_CELLS:-}" = A1a ] || { printf '%s\n' 'ATOMIC REFUSAL P3152_CELLS must equal A1a' >&2; return 2; }
  [ -z "${PODIUM_RUNTIME_DRIVER:-}" ] || { printf '%s\n' 'ATOMIC REFUSAL PODIUM_RUNTIME_DRIVER is set' >&2; return 2; }
  local derived_state
  derived_state="$(env -u PODIUM_PORT -u PODIUM_STATE_DIR -u PODIUM_AGENT_HOME -u PODIUM_HOME -u PODIUM_RUNTIME_DRIVER -u ABDUCO_SOCKET_DIR -u XDG_STATE_HOME PODIUM_INSTANCE="$P3152_INSTANCE" PODIUM_NO_RELAY=1 "$BUN" --conditions=@podium/source -e 'import { instanceStateDir } from "@podium/runtime/instance"; console.log(instanceStateDir())')"
  [ "$P3152_STATE_DIR" = "$derived_state" ] || {
    printf '%s\n' "ATOMIC REFUSAL noncanonical named state root: got=$P3152_STATE_DIR want=$derived_state" >&2; return 2
  }
  [ ! -e "$P3152_STATE_DIR" ] || { printf '%s\n' "ATOMIC REFUSAL fresh r10 whole state root already exists: $P3152_STATE_DIR" >&2; return 2; }
  cleanup_armed=1
  trap finish EXIT

  bash "$RIG" up terminal
  require_live_pid server "$P3152_RUN_DIR/server.pid"
  require_live_pid daemon "$P3152_RUN_DIR/daemon.pid"
  local ports
  ports="$(env -u PODIUM_PORT -u PODIUM_STATE_DIR -u PODIUM_RUNTIME_DRIVER -u ABDUCO_SOCKET_DIR PODIUM_INSTANCE="$P3152_INSTANCE" PODIUM_NO_RELAY=1 "$BUN" --conditions=@podium/source -e 'import { defaultInstancePorts } from "@podium/runtime/instance"; const p=defaultInstancePorts(process.env.PODIUM_INSTANCE); console.log(`${p.server} ${p.hook} ${p.agentRelay}`)')"
  [ "$ports" = "$P3152_EXPECTED_PORTS" ] || { printf '%s\n' "ATOMIC REFUSAL changed r10 port tuple: got=$ports want=$P3152_EXPECTED_PORTS" >&2; return 2; }
  local port
  for port in $ports; do
    case "$port" in 19797|32090) printf '%s\n' "ATOMIC REFUSAL reserved derived port $port" >&2; return 2 ;; esac
    require_listener "$port"
  done
  P3152_CELLS=A1a "$BUN" "$DRIVE" terminal
}

test_invocation() {
  local marker="$1" rig="$2" drive="$3"
  P3152_ATOMIC_TEST_MODE=1 P3152_ATOMIC_TEST_RIG="$rig" P3152_ATOMIC_TEST_MARKER="$marker" \
...
static_self_test() {
  ...
  if [ "${P3152_ATOMIC_TEST_MODE:-0}" = 1 ]; then
    [ -f "${P3152_ATOMIC_TEST_MARKER:?test marker is required}" ] || { printf '%s\n' "STATIC SELF TEST fixture file missing" >&2; return 1; }
  fi
  new_instance=p3152-grok-reply-0d180cc-r10
  old_instance=p3152-grok-reply-control
  new_state="${P3152_STATE_DIR:?P3152_STATE_DIR is required}"
  ...
  [ "$new_ports" = "${P3152_EXPECTED_PORTS// /,}" ] || { printf '%s\n' "STATIC SELF TEST changed frozen r10 ports: got=$new_ports want=${P3152_EXPECTED_PORTS// /,}" >&2; return 1; }
  ...
  printf '%s\n' "ATOMIC_STATIC_SELF_TEST_OK canonical-state=resolver-equal old-new-instance=distinct old-new-state=distinct whole-root-before=absent whole-root-after=absent old-new-ports=distinct frozen-ports=37325,37326,37327 new-ports-pairwise=distinct new-ports-nonreserved=yes new-ports-listeners=zero ports=$new_ports"
}

case "${1:-}" in
  --static-self-test) static_self_test ;;
  --test-child)
    [ "${P3152_ATOMIC_TEST_MODE:-0}" = 1 ] || exit 2
    cleanup_armed=1
    trap finish EXIT
    "$2" "$PROTECTED_MARKER"
    ;;
