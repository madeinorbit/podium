/**
 * POD-3152 — Grok-only release acceptance drive.
 *
...
import {
  ...
  until,
  wait,
} from '/home/mgw/src/podium/.worktrees/issue-3152-grok-reply-repair-proof/docs/evidence/pod-2777/rig'
import type { AskRow, SessionRow } from '/home/mgw/src/podium/.worktrees/issue-3152-grok-reply-repair-proof/docs/evidence/pod-2777/rig'
...
const EXPECTED_AGENT_HOME = process.env.P3152_AGENT_HOME ?? (() => { throw new Error('P3152_AGENT_HOME is required') })()
const EXPECTED_STATE_DIR = process.env.P3152_STATE_DIR ?? (() => { throw new Error('P3152_STATE_DIR is required') })()
const EXPECTED_GROK_HOME = join(EXPECTED_AGENT_HOME, '.grok')
const EXPECTED_ABDUCO_SOCKET_DIR = process.env.P3152_ABDUCO_SOCKET_DIR ?? (() => { throw new Error('P3152_ABDUCO_SOCKET_DIR is required') })()

const INSTANCE = 'p3152-grok-reply-0d180cc-r10'
const PRODUCT_PIN = '0d180cc0455832ffe93edf2ac450a47f5f7c8137'
const RUN_TOKEN = process.env.P3152_RUN_TOKEN ?? (() => { throw new Error('P3152_RUN_TOKEN is required') })()
...
function parseCellSelector(raw: string | undefined): Set<CanonicalCellId> {
  ...
  if (raw === undefined || raw === '') throw new Error('P3152_CELLS is required; use exact comma-separated ids or ALL')
  const tokens = raw.split(',').map((token) => token.trim())
  if (tokens.some((token) => token === '')) throw new Error('P3152_CELLS contains an empty token')
  const selected = new Set<CanonicalCellId>()
  for (const token of tokens) {
    if (!CANONICAL_CELL_SET.has(token)) throw new Error(`P3152_CELLS unknown id: ${token}`)
    const id = token as CanonicalCellId
    if (selected.has(id)) throw new Error(`P3152_CELLS duplicate id: ${id}`)
    selected.add(id)
  }
  return selected
}
...
const SELECTED_CELLS = parseCellSelector(process.env.P3152_CELLS)
...
function evidenceFields(cell: Cell): string[] {
  ...
  const fields = [
    ...
    new Date(cell.at).toISOString().replace('T', ' ').replace('Z', ' UTC'), 'POD-3152',
  ].map(field)
...
function appendAuthoritativeRow(cell: Cell, fields: string[], readingPath: string, pinPath: string): void {
  ...
  if (prior.split('\n').some((line) => line.startsWith(`${identity}\t`) && line.endsWith('\tPOD-3152'))) {
    throw new Error(`refusing duplicate POD-3152 run identity: ${identity}`)
  }
  ...
  const committed = spawnSync('git', ['commit', '-m', `test(evidence): record Grok ${arm} ${cell.id} ${RUN_TOKEN}`, '-m', 'Podium-Issue: POD-3152'], { cwd: REPO, encoding: 'utf8' })
...
class RichChat {
  ...
  async open(): Promise<void> {
    const auth = await fetch(`${BASE}/auth/login`, {
      ...
      body: JSON.stringify({ password: process.env.PODIUM_PASSWORD ?? 'p3152-grok-reply-r10' }),
    })
...
async function createSession(id: string, suffix = ''): Promise<{ sid: string; cwd: string; response: any }> {
  ...
  const response = await mutate('sessions.create', {
    ...
    runtimeContract: arm === 'headless' ? 'grok-acp' : 'generic-pty',
  })
...
async function openSession(id: string, suffix = ''): Promise<SessionContext> {
  ...
  try {
    ...
    const bindingOk = arm === 'headless'
      ...
      : row.requestedDriverId === 'generic-pty' && row.driverId === 'generic-pty' && row.driverFamily === 'terminal'
...
function hasExactInstanceEnvironmentEntry(raw: string, instance: string): boolean {
  return raw.split('\0').includes(`PODIUM_INSTANCE=${instance}`)
}
...
function ownedPids(): { pids: number[]; cmds: Map<number, string> } {
  ...
  for (const name of readdirSync('/proc')) {
    ...
    try {
      ...
      if (!hasExactInstanceEnvironmentEntry(env, INSTANCE)) continue
      const cmd = readFileSync(`/proc/${name}/cmdline`, 'utf8').replace(/\0/g, ' ').trim()
      if (/scripts\/(daemon|server)\.ts|grok-drive\.ts|docs\/evidence\/pod-3152\/drive\.ts/.test(cmd)) continue
...
if (process.env.P3152_STATIC_SELF_TEST === '1') {
  ...
  const token = 'P3152_UNIQUE_REPLY'
  ...
  const exactInstanceEntry = `PODIUM_INSTANCE=${INSTANCE}`
  if (!hasExactInstanceEnvironmentEntry(`PATH=/bin\0${exactInstanceEntry}\0HOME=/tmp\0`, INSTANCE)) {
    throw new Error('exact PODIUM_INSTANCE environment entry was rejected')
  }
  for (const nearMiss of [
    `NEAR=${exactInstanceEntry}\0`,
    `PODIUM_INSTANCE=${INSTANCE}-near\0`,
    `PREFIX_${exactInstanceEntry}\0`,
  ]) {
    if (hasExactInstanceEnvironmentEntry(nearMiss, INSTANCE)) throw new Error(`near-miss instance environment was accepted: ${nearMiss}`)
  }
