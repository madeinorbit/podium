#!/usr/bin/env bash
# POD-3152 — isolated Grok acceptance rig.
#
...
REPO="$(cd "$HERE/../../.." && pwd)"
INSTANCE="p3152-grok-reply-0d180cc-r10"
EXPECTED_PORTS="37325 37326 37327"
DRIVE_BASE="/tmp/pod-3152-grok-reply-0d180cc-r10"
RUN_TOKEN="${P3152_RUN_TOKEN:?source rig-env.sh to set an immutable UTC run token}"
RUN_DIR="$DRIVE_BASE/runs/$RUN_TOKEN"
LOGS="$RUN_DIR/logs"
WEB="$REPO/apps/web/dist"
BUN="/home/mgw/.bun/bin/bun"
PASSWORD="p3152-grok-reply-0d180cc-r10-proof"
...
[ "$PORT $HOOK_PORT $RELAY_PORT" = "$EXPECTED_PORTS" ] || { printf '%s\n' "refusing changed r10 port tuple: got=$PORT $HOOK_PORT $RELAY_PORT want=$EXPECTED_PORTS" >&2; exit 1; }
...
start_component() {
  ...
  printf '%s\n' 0d180cc0455832ffe93edf2ac450a47f5f7c8137 >"$RUN_DIR/$name.sha"
...
verify() {
  ...
  want_sha=0d180cc0455832ffe93edf2ac450a47f5f7c8137
  ...
  log "PIN OK row=$row arm=$arm instance=$INSTANCE serverPid=$server_pid daemonPid=$daemon_pid serverSha=$want_sha webSourceSha=$web_sha driver=per-session-runtime-contract grokVersion='grok 0.2.118 (1e1687c1cf) [stable]' grokBinSha256=c192282e62abd24a9be64750363ff827d806ba613918399a8c69c815b1da08f6 state=$STATE_DIR home=$NORMAL_HOME"
