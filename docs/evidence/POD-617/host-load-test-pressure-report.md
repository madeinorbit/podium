...
- The durable integration combines nested lease ownership, durable waiters, queued lease metadata, clearer residency/idle signals, and a shared validation admission budget. The final full test, typecheck, and integrated UI verification remain the parent gate.
...
- POD-623 is validated and pending integration: queued waiters retain their requested TTL and note instead of receiving the two-minute default on grant.
- POD-618 is validated and review-ready, pending integration: focused tests and typechecks enter a shared admission budget, with ownership propagated to nested package tasks so they do not deadlock or self-release.
