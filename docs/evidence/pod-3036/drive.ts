/**
 * POD-3036 — Claude SDK vs PTY acceptance at the current 1761 tip.
 *
 *   bun docs/evidence/pod-3036/drive.ts A1a claude-sdk
 *   bun docs/evidence/pod-3036/drive.ts A1a claude-pty
 *
...
const BASE = process.env.PODIUM_DRIVE_BASE ?? '/tmp/pod-3036'
const PORT = process.env.PODIUM_PORT ?? '19936'
const ROOT = process.cwd()
const READING_DIR = join(ROOT, 'docs/evidence/pod-3036/readings')
const PIN_DIR = join(ROOT, 'docs/evidence/pod-3036/pins')
const AGENT_HOME =
  process.env.P3036_STATE_ROOT
    ? join(process.env.P3036_STATE_ROOT, 'agent-home')
    : join(process.env.HOME ?? '', '.local/state/podium/p3036/agent-home')
const REPLY_MS = Number(process.env.P3036_REPLY_MS ?? 180_000)
const BUSY_MS = Number(process.env.P3036_BUSY_MS ?? 90_000)
const STEP_MS = 500
...
const CELLS = new Set([
  ...
  'A6A', 'A6B', 'A7A', 'A7B', 'A8', 'A9', 'A10', 'BQUOTA', 'BAUTH',
])
...
async function pinFor(label: string): Promise<Pin> {
  ...
  const pinSha = process.env.P3036_PIN_SHA ?? 'c71b896a9504426ee6706a0623e9cb25e704cc98'
...
async function create(): Promise<{ sid: string; chat: Chat; created: unknown; row?: Status }> {
  ...
  if (!existsSync(join(cwd, '.git'))) {
    ...
    writeFileSync(join(cwd, 'README.md'), `POD-3036 ${driver} ${cell}\n`)
...
async function baselineReply(sid: string, chat: Chat, tag: string) {
  const needle = 'P3036-' + tag + '-' + Date.now().toString(36).toUpperCase()
...
function instanceProcesses(): ProcessRow[] {
  const inst = process.env.PODIUM_INSTANCE ?? 'p3036'
  const rows: ProcessRow[] = []
...
async function runA1a() {
  ...
  try {
    for (let i = 1; i <= 3; i++) {
      const needle = 'P3036-A1A-' + driver + '-' + i + '-' + Date.now().toString(36).toUpperCase()
      const sent = await mutate('sessions.sendText', {
        sessionId: sid,
        text: 'Reply with exactly this word and nothing else: ' + needle + '. Do not use tools.',
      })
...
async function runA1b() {
  const { sid, chat } = await create()
  try {
    const first = 'P3036-A1B-FIRST-' + Date.now().toString(36).toUpperCase()
    const queued = 'P3036-A1B-QUEUED-' + Date.now().toString(36).toUpperCase()
...
async function runA1c() {
  ...
  try {
    ...
    const dead = 'P3036-A1C-DEAD-' + Date.now().toString(36).toUpperCase()
...
async function runA2a() {
  const { sid, chat } = await create()
  try {
    const needle = 'P3036-A2A-' + Date.now().toString(36).toUpperCase()
    const sent = await mutate('sessions.sendText', {
      sessionId: sid,
...
async function runA3() {
  ...
  const { sid, chat } = await create()
  try {
    const needle = 'P3036-A3-' + Date.now().toString(36).toUpperCase()
    const sent = await mutate('sessions.sendText', {
      sessionId: sid,
...
async function runA4a() {
  const { sid, chat } = await create()
  try {
    const marker = 'P3036-A4A-' + Date.now().toString(36).toUpperCase()
    const outside = join('/tmp', 'pod-3036-external')
...
async function runA4b() {
  const { sid, chat } = await create()
  try {
    const marker = 'P3036-A4B-' + Date.now().toString(36).toUpperCase()
    const outside = join('/tmp', 'pod-3036-external')
    mkdirSync(outside, { recursive: true })
    const sent = await mutate('sessions.sendText', {
      sessionId: sid,
...
async function runA5() {
  const { sid, chat } = await create()
  try {
    const marker = 'P3036-A5-MARKER-' + Date.now().toString(36).toUpperCase()
...
async function runA6a() {
  ...
  try {
    ...
    const marker = 'P3036-A6A-' + Date.now().toString(36).toUpperCase()
...
async function runA6b() {
  ...
  try {
    ...
    const cli1 = 'P3036-A6B-CLI1-' + Date.now().toString(36).toUpperCase()
    ...
    view(chat, sid, 'native')
    await wait(1_500)
    const cli2 = 'P3036-A6B-CLI2-' + Date.now().toString(36).toUpperCase()
...
async function restartDaemon(): Promise<{ oldPid: string; newPid: string; out: string }> {
  ...
  const r = spawnSync('bash', [join(ROOT, 'docs/evidence/pod-3036/restart-daemon.sh')], { encoding: 'utf8', env: process.env })
...
async function runA7a() {
  const { sid, chat } = await create()
  try {
    const secret = 'P3036-A7A-' + Date.now().toString(36).toUpperCase()
    const sent = await mutate('sessions.sendText', {
      sessionId: sid,
...
async function runA7b() {
  const { sid, chat } = await create()
  try {
    const secret = 'P3036-A7B-' + Date.now().toString(36).toUpperCase()
...
async function runBquota() {
  const { sid, chat } = await create()
  try {
    const needle = 'P3036-BQUOTA-' + Date.now().toString(36).toUpperCase()
    const sent = await mutate('sessions.sendText', {
      sessionId: sid,
      text: 'Reply with exactly this word and nothing else: ' + needle + '. Do not use tools.',
    })
