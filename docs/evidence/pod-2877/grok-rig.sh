#!/usr/bin/env bash
# POD-2877 — isolated Grok acceptance rig.
#
...
seed_grok() {
  ...
  if [ ! -f "$AGENT_HOME/.grok/auth.json" ]; then
    ...
    cp "$NORMAL_HOME/.grok/auth.json" "$AGENT_HOME/.grok/auth.json"
    chmod 600 "$AGENT_HOME/.grok/auth.json"
  fi
}
...
up() {
  ...
  wait_health
  if [ "$arm" = terminal ]; then
    start_component daemon scripts/daemon.ts PODIUM_RUNTIME_DRIVER=generic-pty
  else
...
verify() {
  ...
  local expected_driver actual_driver
  actual_driver="$(printf '%s\n' "$daemon_env_text" | sed -n 's/^PODIUM_RUNTIME_DRIVER=//p' | tail -1)"
  if [ "$arm" = terminal ]; then expected_driver=generic-pty; else expected_driver=; fi
  [ "$actual_driver" = "$expected_driver" ] || { log "PIN FAIL driver=$actual_driver want=${expected_driver:-<policy>}"; return 1; }
  log "PIN OK row=$row arm=$arm instance=$INSTANCE serverPid=$server_pid daemonPid=$daemon_pid serverSha=$want_sha webSourceSha=$web_sha driver=${actual_driver:-policy} state=$STATE_DIR home=$NORMAL_HOME"
