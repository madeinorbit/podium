...
export function providerError(harness: string): Probe {
  return {
    ...
    async run(ctx) {
      ...
      /**
       * A FAULT THAT ACTUALLY FIRES, supplied by POD-1761 from POD-2772/POD-2604.
       *
       * The first version named a nonsense model, and opencode simply IGNORED
       * the string and answered "Hello!" — no provider ever refused, so there was
       * no error to surface and the cell measured nothing. Correctly scored n/a,
       * and useless.
       *
       * `opencode/laguna-s-2.1-free` is different in exactly the way this row
       * needs: the model has been RETIRED from opencode's gateway, so the session
       * BINDS, is marked LIVE, `gateway.send` returns ACCEPTED with a protocol-ack
       * and a turnEpoch — and then nothing ever settles. No error arrives, the
       * phase never moves, and the only signal is a timeout. Accepted at the
       * boundary, then silence. It needs no quota exhaustion and no revoked
       * credential, and it reproduces on demand.
       *
       * That is precisely what this cell exists to test: what a user sees when a
       * provider fails silently. A product that says nothing here is one where a
       * turn that died renders as a session sitting patiently at idle.
       *
       * PER HARNESS: the retired id is an OPENCODE gateway model, so it is only a
       * fault on opencode. Where no equivalent accepted-then-never-settles fault
       * is known for a harness, this probe reports n/a with that reason rather
       * than firing a fault that does not fire — a fixture must produce the thing
       * it claims to test.
       */
      ...
      if (!bogus) {
        return {
          ...
          outcome: {
            ...
            evidence: [
              `HARNESS           ${harness}`,
              'The fault this cell needs is one the provider ACCEPTS and then never',
              'settles — opencode/laguna-s-2.1-free does that on opencode (retired',
              'from the gateway: binds, marked live, ACCEPTED with a turnEpoch, then',
              'silence; POD-2604). No equivalent is known for this harness, and a',
              'nonsense model string is NOT one: opencode ignored it entirely and',
              'answered normally, which measures nothing about error surfacing.',
              'n/a with the reason beats a fault that does not fire.',
            ],
      ...
      if (!honest && answered.length > 0) {
        return {
          ...
          outcome: {
            ...
            evidence: [
              `INJECTED FAULT    model="${bogus}" — RETIRED from the gateway (POD-2604)`,
              `SESSION           ${sid}`,
