...
- Server-side suppression: the handshake opts out of `DELTA_NOTIFICATIONS` (`item/agentMessage/delta`, `item/reasoning/textDelta`, `item/reasoning/summaryTextDelta`, `item/plan/delta`; `protocol.ts:190-204`) when at coarse (`drivers/codex/runtime.ts:1626-1639`). Consequently **upgrading to fine is a reconnect**: `upgradeToFine` (`runtime.ts:1491-1540`) is single-flight, refuses while busy or with asks outstanding, respawns + `thread/resume`s; downgrade is deliberately never done (`runtime.ts:1250-1262`).
...
- **Fine-watch lifecycle:** the daemon acquires `watch('fine')` for a session **while at least one client transcript subscription exists** for it, plumbed as a subscriber-count signal server→daemon (a small control frame alongside `transcriptSubscribe`/`transcriptUnsubscribe`, `client-control.ts:183-190`). Codex's upgrade-is-a-reconnect constraint (§1.2) means the upgrade may land mid-turn or be refused while busy — acceptable: streaming simply begins on the next turn. Release with a debounce (~30 s) to avoid flapping; note codex never downgrades anyway (`runtime.ts:1250-1262`). See OQ-2 for the simpler always-fine alternative.
...
- **OQ-2 — Fine-watch lifecycle: subscriber-driven vs always-fine.** Always-fine is simpler (no server→daemon signal) but codex pays a respawn/reconnect per session and every session pays per-token emit+journal cost with nobody watching. *Recommendation: subscriber-driven with release debounce (§3.2); if the control-frame plumbing proves heavy, fall back to always-fine for opencode/grok (their deltas are free on the wire) and subscriber-driven for codex only.*
