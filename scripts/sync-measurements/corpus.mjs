...
export function rowPayload(index) {
  ...
  let unique = ''
  for (let i = 0; i < 2200; i++) { state ^= state << 13; state ^= state >>> 17; state ^= state << 5; unique += alphabet[state >>> 26] }
  ...
  try {
    ...
    for (let start = 0; start < rows; start += 128) {
      await store.sync.appendChanges(Array.from({ length: Math.min(128, rows - start) }, (_, offset) => ({
        entity: 'repo', entityId: `measurement-repo-${start + offset}`, op: 'upsert', payload: rowPayload(start + offset),
      })), Date.now())
    }
    ...
    for (let start = 0; start < deltaRows; start += 128) {
      await store.sync.appendChanges(Array.from({ length: Math.min(128, deltaRows - start) }, (_, offset) => ({
        entity: 'repo', entityId: `measurement-repo-${(start + offset) % rows}`, op: 'upsert', payload: rowPayload((start + offset) % rows),
      })), Date.now())
    }
    ...
    const manifest = { schema: 2, seed: 'xorshift32:6d2b79f5', entropyCharacters: 2200, payloadSha256: digest.digest('hex'), targetZstdRatio: [5,7], rows, deltaRows, from, through, scopedPayloadBytes,
      payloadBytesPerRow: Buffer.byteLength(payload), shape: 'public repo rows, unique seeded entropy plus repeated text; target roughly 6x Zstd',
