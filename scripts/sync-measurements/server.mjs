...
const { NativeGatewaySocket, NATIVE_BACKPRESSURE_LIMIT_BYTES } = await load('apps/server/src/gateway/ws-server.ts')
...
const app = new Hono()
app.post('/checkpoint', c => c.json(checkpointDb.query(new URL(c.req.url).searchParams.get('truncate')==='1'?'PRAGMA wal_checkpoint(TRUNCATE)':'PRAGMA wal_checkpoint(PASSIVE)').get()))
...
if (arm === 'after') {
  ...
  worker.worker.on('message', message => {
    ...
    if(!state)return
    if(message.type==='meta'){state.rssAtMetaBytes=process.memoryUsage().rss;state.metaAt=performance.now()}
    if(message.type==='bytes'){state.bytes+=message.chunk.byteLength;state.chunks++;state.maxChunkBytes=Math.max(state.maxChunkBytes,message.chunk.byteLength);state.lastProgressAt=performance.now()}
    if(message.type==='metrics')state.metrics=message.metrics
    if(message.type==='end'||message.type==='error'){state.complete=true;state.reason=message.reason;state.completedAt=performance.now()}
  })
  const measuredWorker = { bootstrap(input, signal) {
    relays[input.transferId]={principal:input.principal.user,bytes:0,chunks:0,complete:false,maxChunkBytes:0,startedAt:performance.now()}
    return worker.bootstrap(input,signal)
  } }
  registerSyncRoutes(app, { authority, worker: () => measuredWorker, serving,
...
app.get('/metrics', c => c.json({ senders: Object.fromEntries(Object.entries(senders).map(([id,sink]) => [id,sink.stats()])), relays, workerMetrics, jobs: worker?.activeJobCount() ?? null, manifest }))
let writeCount=0
app.post('/write', async c => {
  await store.sync.appendChanges([{ entity: 'repo', entityId: 'measurement-repo-0', op: 'upsert', payload: rowPayload(100000 + writeCount++) }], Date.now())
...
const server = Bun.serve({ hostname: '127.0.0.1', port: 0,
  fetch(request, instance) {
    ...
    if (url.pathname === '/ping' || url.pathname === '/bootstrap-ws') {
      if (instance.upgrade(request, { data: { id: url.searchParams.get('client') ?? crypto.randomUUID(), bootstrap: url.pathname === '/bootstrap-ws', coding: url.searchParams.get('coding') } })) return
    }
    return app.fetch(request)
  }, websocket: {
    backpressureLimit: NATIVE_BACKPRESSURE_LIMIT_BYTES, closeOnBackpressureLimit: false, idleTimeout: 0, sendPings: false,
    open(ws) {
      ...
      serving.attach({ id: ws.data.id, wireVersion: 2, acceptsDelta: true, send: sink.send, sendSequence: sink.sendSequence, terminate: () => ws.terminate() }, principal, principal)
    }, drain(ws) { ws.data.native?.emit('drain') }, message() {}, close(ws) { ws.data.sink?.dispose(); serving.detach(ws.data.id) },
  },
})
