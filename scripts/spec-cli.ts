...
import { SPEC_COMMANDS } from '../apps/server/src/spec-commands'
import { IssueCliError, parseIssueArgs } from './issue-cli'

/**
 * `podium spec` — agent/operator CLI over the living project spec (pspec v1).
 * Same shape and transports as `podium issue` (scripts/issue-cli.ts): relayed
 * through the daemon when PODIUM_ISSUE_RELAY is set (agent path), direct tRPC
 * otherwise (operator path). Commands live in apps/server/src/spec-commands.ts.
 */
...
/** Entry used by scripts/cli.ts — same transport selection as issueCliMain. */
