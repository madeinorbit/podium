 ...
 * + scripts/daemon.ts). Runs on Bun (preferred) or Node via tsx — the PTY backend is selected
 * at runtime (@podium/agent-bridge): Bun.Terminal under Bun, node-pty under Node, so no native
 * addon is loaded on Bun.
 *   bun --watch --conditions=@podium/source scripts/host.ts   # or: bun run host:backend
