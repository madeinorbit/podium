...
export const ERROR_LEVEL_MANIFEST_RULES: ReadonlySet<string> = new Set([
  'harness-branching',
  'harness-classifier-boundary',
  // POD-333: a retired path has no migration to ratchet down — see
  // {@link RETIRED_MODULES}. Its correct count is zero on the commit that
  // declares it, so an allowlist entry could only bless a regression.
  'manifest-retired-path',
])
