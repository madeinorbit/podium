 ...
 *  2. `@podium/agent-bridge` and `@podium/harness` may only be imported by `apps/daemon`, `scripts/`,
...
export function checkFile(
  ...
  for (const ref of extractImports(source)) {
    ...
    // Rule 2: agent-bridge / harness importers are restricted. POD-397 split the
    // package, so the restriction follows BOTH halves — otherwise the split would
    // have quietly opened a door that was closed before it. Kept under the ONE
    // existing rule id ('agent-bridge-consumers') on purpose: the ratchet in
    // scripts/boundary-allowlist.ts keys on (rule, file), so a per-file count that
    // was 1 before the split is still 1 after it, and the debt neither resets nor
    // double-counts. POD-399 renames the rule when it deletes agent-bridge.
    if (to === 'packages/agent-bridge' || to === 'packages/harness') {
      const allowed =
        from === 'apps/daemon' ||
        from === 'scripts' ||
        from === 'packages/agent-bridge' ||
      ...
      violations.push({
        ...
        message: `${file}: '${ref.specifier}' may only be imported by apps/daemon, scripts/, or its own tests (Phase 3 extracts @podium/transcript for the grandfathered server cases)`,
      })
    }
  }
  return violations
