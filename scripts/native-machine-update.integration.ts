...
const output = resolve(process.argv[2] ?? '.tmp/native-machine-update-evidence.json')
...
async function until<T>(
  ...
  throw new Error(
    `${label}: ${JSON.stringify(last)}; native log: ${readFileSync(log, 'utf8').slice(-6000)}`,
  )
}
...
function shellPids() {
  return readdirSync('/proc')
    ...
    .flatMap((name) => {
      try {
        return readFileSync(`/proc/${name}/cmdline`, 'utf8').split('\0')[0] === app
          ? [Number(name)]
          : []
      } catch {
        return []
...
try {
  ...
  run(signer, ['signer', 'generate', '--ci', '--password', '', '--write-keys', key])
  const pubkey = readFileSync(key + '.pub', 'utf8').trim()
  const resource = join(tauri, 'resources/payload')
  ...
  for (const [version, destination] of [
    ...
    run('cargo', ['build', '--manifest-path', join(tauri, 'Cargo.toml')], {
      ...
      TAURI_CONFIG: JSON.stringify({ version, plugins: { updater: { pubkey } } }),
    })
    cpSync(join(tauri, 'target/debug/Podium'), destination)
    chmodSync(destination, 0o755)
  }
  run(signer, ['signer', 'sign', '--private-key-path', key, '--password', '', target])
  ...
  server = Bun.serve({
    ...
    fetch(request) {
      ...
      if (path === '/artifact') return new Response(targetBytes)
      if (path === '/manifest')
  ...
  cpSync(join(repo, 'dist-bun/targets/linux-x86_64/headless'), payload, { recursive: true })
  run(process.execPath, [
    ...
    join(repo, 'scripts/cli-compiled.ts'),
  ...
  if (existsSync(join(state, 'run/daemon.pid')) || existsSync(join(state, 'run/server.pid')))
  ...
  const grant = (sig: string) => ({
    ...
    target: {
      version: newVersion,
      critical: false,
      native: { url: origin + '/artifact', signature: sig, version: newVersion, channel: 'dev' },
      artifacts: {
        desktop: {
          delivery: 'feed',
          platforms: {
  ...
  if (digest(app) !== oldDigest)
    throw new Error('verification failure changed the running shell artifact')
  ...
  const journal = await until(
    () => readMachineUpdateJournal(runtime),
    (journal) => {
      if (journal && observed.at(-1) !== journal.phase) observed.push(journal.phase)
  ...
  if (digest(app) !== digest(target) || journal?.prepared?.digest !== digest(target))
    ...
    JSON.stringify(
      {
        ...
        observed,
        scope:
          'Real native verifier, Linux executable install primitive, app restart and supervisor recovery. Debug ELF builds; does not cover AppImage packaging/FUSE, macOS bundles or Windows installers.',
      },
      null,
      2,
    ),
  ...
  try {
    const endpoint = JSON.parse(readFileSync(join(runtime, 'machine-update-control.json'), 'utf8'))
