...
export interface FleetCommandContract<In extends z.ZodTypeAny = z.ZodTypeAny, Out = unknown>
  ...
  readonly policy: CommandPolicy & {
    readonly machineSharingAuthority?: 'owner-or-admin'
...
/** Share one machine verb. Owners and admins may change the audience. */
export const machineShareContract = {
  ...
  policy: {
    ...
    confirmation: 'none',
    rationale:
      'Sharing widens who may see, use, or manage someone’s compute. The member floor keeps the owner reachable, the manage check hides invisible rows, and owner-or-admin authority permits administrative custody while refusing a delegated manage grantee.',
  },
  exposure: SERVED_ON,
  delivery: FLEET_DELIVERY,
  redaction: PUBLIC_REDACTION,
  ownership: {
    ...
    visibility: 'owned-compute',
    inheritanceOnCreate: 'parent',
    note: 'The edge is subordinate to the machine and records the authenticated human as grantor.',
  },
  attribution: FLEET_ATTRIBUTION,
  errorConsistency: {
    callerSuppliedTargetId: true,
    invisibleFailsAs: 'nonexistent',
    distinguishesUnauthorizedFromUnreachable: false,
    note: 'An invisible machine and an unknown id share one refusal; a visible non-owner is told sharing requires the owner or an admin.',
  },
  serverRole: 'hub',
...
/** Revoke exactly one machine grant. The same owner-or-admin authority applies. */
export const machineUnshareContract = {
  ...
  policy: {
    ...
    confirmation: 'none',
    rationale:
      'Revocation changes the machine audience and is reserved to the direct owner or an admin for the same reason as sharing; a delegated manage grant is not authority to rewrite delegation.',
  },
  exposure: SERVED_ON,
  delivery: FLEET_DELIVERY,
  redaction: PUBLIC_REDACTION,
  ...
  errorConsistency: {
    ...
    invisibleFailsAs: 'nonexistent',
    distinguishesUnauthorizedFromUnreachable: false,
    note: 'The same visibility and owner-or-admin refusal shape as machines.share.',
  },
  serverRole: 'hub',
...
/** Transfer custody explicitly. An admin transferring to themselves is an audited takeover.
 * Role-based manage never itself grants use; transfer changes the personal grantee. */
export const machineTransferOwnershipContract = {
  ...
  policy: {
    ...
    confirmation: 'none',
    rationale:
      'Transfer moves the root of a machine’s access graph to another person: the new owner gains see, use and manage by default and the old owner keeps none of them. The member floor keeps ordinary owners able to hand over their own hardware, the manage check keeps an invisible machine invisible, and owner-or-admin authority permits an audited admin takeover while excluding a delegated manage grantee.',
  },
  exposure: SERVED_ON,
  delivery: FLEET_DELIVERY,
  redaction: PUBLIC_REDACTION,
  ownership: {
    creates: ['machine custody audit event'],
    owner: 'on-behalf-of-human',
    visibility: 'owned-compute',
    inheritanceOnCreate: 'parent',
    note: 'Changes the database personal grantee, removes existing grant edges and appends an audit event atomically. Admin transfer to self records takeover.',
  },
  attribution: FLEET_ATTRIBUTION,
  errorConsistency: {
    callerSuppliedTargetId: true,
    invisibleFailsAs: 'nonexistent',
    distinguishesUnauthorizedFromUnreachable: false,
    note: 'An invisible machine and an unknown id share one refusal; a visible non-owner is told transfer requires the owner or an admin. The recipient id is caller-supplied too, and an unknown recipient is refused rather than silently quarantining the machine.',
  },
  serverRole: 'hub',
  cli: { summary: 'Transfer machine ownership to another user' },
  conflict: 'cmd',
  conflictRule:
    'One machine transition and database transaction order custody, grant removal and audit together',
} as const satisfies FleetCommandContract<typeof machineTransferOwnershipInput>

/** Adopt an explicitly unowned machine. Existing custody changes through transfer. */
export const machineAdoptContract = {
  ...
  ownership: {
    ...
    visibility: 'owned-compute',
    inheritanceOnCreate: 'parent',
    note: 'An admin explicitly names the recipient or adopts to themselves. The database transaction changes custody, removes surviving grants and records the audit event together.',
  },
  attribution: FLEET_ATTRIBUTION,
  errorConsistency: {
    callerSuppliedTargetId: true,
