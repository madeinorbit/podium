...
describe('the derived path sets find something', () => {
  ...
  it('the writable set is both preference tiers and NOTHING from the secret tier', () => {
    // 44 = 28 personal + 16 instance, including transcripts.mirror. Secrets stay out.
    expect(WRITABLE_PREFERENCE_PATHS.length).toBe(44)
    expect(WRITABLE_PREFERENCE_PATHS).toContain('transcripts.mirror')
