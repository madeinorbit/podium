...
const createInput = z.object({
  ...
  /**
    ...
    * renamed POD-4426).
    *
    * A DRIVER ID names the engine for this spawn explicitly, which is how a
    * single opencode session runs on `opencode-server` while every other
    * session on the same daemon stays terminal. Absent is the manifest's
    * headed terminal default and changes nothing; the daemon binds a driver
    * for every profiled agent unconditionally. The old `true` spelling (delegate
    * to the manifest policy) is gone because it named the default the field's
