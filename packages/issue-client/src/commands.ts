 ...
 * `podium machine show` and `podium session handoff --to` use — so no two
 * surfaces can come to disagree about which host a name means. It is exact by
 * design: a wrong resolution starts real work on the wrong box, where it looks
 * like it worked.
 *
 * The list it resolves against is already scoped to what this caller may see, so
 * an id typed for a machine the caller cannot see is refused as unknown — which
 * is also the answer that leaks nothing about whether it exists.
 */
...
export const ISSUE_COMMANDS: IssueCommand[] = [
  ...
  },
  {
    ...
    async run(c, a) {
      // `--machine` takes a NAME as well as an id (POD-1386). It used to pass the
      // raw string through as `machineId`, which meant a caller had to know a uuid
      // that nothing printed; `podium machine list` prints the names.
  ...
  },
  {
    ...
    positionals: ['id'],
    async run(c, a) {
      // `--machine` HOMES the issue rather than passing a one-shot placement, and
      // that is the honest model: `issues.machine_id` is where an issue lives, and
      // every later spawn on it (`agent spawn --issue`) must land in the SAME
      // worktree. A per-start override would let the pin and the worktree disagree
      // — one issue with two homes — which is the divergence this whole issue is
      // about. So resolve the name, write the pin, then start.
      if (a.machine) {
  ...
  },
  {
    ...
    summary:
      ...
      'suggested answers rendered as chips — in the Task dock on web, the Tray on ' +
      'mobile. asked-by defaults to your own ' +
  ...
  },
  {
    ...
    async run(c, a) {
      ...
      // Git tracking is deliberately not reported (POD-1284): the bytes live in
      // the server's permanent store, so an uncommitted source file is normal.
