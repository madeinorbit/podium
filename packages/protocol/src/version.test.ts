...
describe('wire protocol version', () => {
  it('CLIENT_WIRE_VERSION is a positive integer', () => {
    expect(Number.isInteger(CLIENT_WIRE_VERSION)).toBe(true)
    expect(CLIENT_WIRE_VERSION).toBeGreaterThan(0)
  })
  it('same version is compatible', () => {
    expect(isProtocolCompatible(CLIENT_WIRE_VERSION, CLIENT_WIRE_VERSION)).toBe(true)
  })
...
describe('version support classification', () => {
  it('MIN_CLIENT_WIRE_VERSION is a positive int ≤ CLIENT_WIRE_VERSION', () => {
    expect(Number.isInteger(MIN_CLIENT_WIRE_VERSION)).toBe(true)
    expect(MIN_CLIENT_WIRE_VERSION).toBeGreaterThan(0)
    expect(MIN_CLIENT_WIRE_VERSION).toBeLessThanOrEqual(CLIENT_WIRE_VERSION)
  })
