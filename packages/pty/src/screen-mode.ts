 ...
 *
 * MOVED from `apps/daemon/src/screen-mode.ts` verbatim: the ONLY signal is
 * DECSET/DECRST 1049 (`CSI ? 1049 h` / `CSI ? 1049 l`) read off the output
 * stream. 1047 is the older switch and the policy does not use it, so it is
 ...
 * backwards.
 *
 * Pure output interpretation: names no SessionId, no protocol frame, no daemon
 * context, so it belongs in `@podium/process/screen`.
 */
