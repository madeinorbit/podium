 ...
 * (POD-3918 P1b).
 *
 * MOVED from `apps/daemon/src/reopen-policy.ts` verbatim (only the
 * `ScreenMode` import is repointed): a pure function over the mode, the model
 * size, the viewer size and three booleans. It names no SessionId, no protocol
 * frame and no daemon context, so it belongs in `@podium/process/screen`
 * where `TerminalScreen` owns it alongside the model it decides about.
 *
