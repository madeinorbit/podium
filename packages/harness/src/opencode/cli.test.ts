...
describe('opencode CLI detection', () => {
  it.skipIf(process.env.PODIUM_REAL_CLI !== '1' || !isOpencodeCliAvailable())(
    '[real-agent:opencode] detects the installed opencode binary',
    () => {
  ...
  it.skipIf(process.env.PODIUM_REAL_CLI !== '1' || !existsSync(homeInstall))(
    '[real-agent:opencode] prefers ~/.opencode/bin over bare opencode on PATH',
    () => {
