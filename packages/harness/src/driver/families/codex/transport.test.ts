 ...
 *
 * (Moved from apps/daemon/src/runtime/codex-app-server.transport.test.ts in
 * 1.5 with the code it pins: the retry loop and the refusal classification
 * are the codex family's protocol edge. The socket library arrives through
 * the `dial` port — here a real `ws` client, in production the supervisor's
 * `dialEngineSocket`, which is pinned separately daemon-side.)
 *
...
/** Production-shaped dial: a real `ws` client over the Unix listener. */
function dial(path: string): Promise<CodexRawSocket> {
  const socket = new WebSocket(`ws+unix://${path}:/rpc`, {
    maxPayload: 128 << 20,
    perMessageDeflate: false,
  })
  socket.on('error', () => undefined)
  return Promise.resolve(socket as unknown as CodexRawSocket)
}
...
describe('connecting to a listener that behaves', () => {
  it('opens on the upgrade and carries JSON-RPC both ways', async () => {
    ...
    const socket = await connect(peer.path, alive, noBanner, 5_000)
    opened.push(socket)
  ...
  it('waits out a listener that is not bound yet', async () => {
    ...
    const socket = await connect(path, alive, noBanner, 5_000)
    opened.push(socket)
  ...
  it('OPENS against a peer that takes 2s over a correct handshake', async () => {
    ...
    const socket = await connect(peer.path, alive, noBanner)
    opened.push(socket)
...
describe('connecting to a listener that cannot complete', () => {
  it('FAILS within the deadline when the peer accepts and then says nothing', async () => {
    ...
    await expect(connect(peer.path, alive, noBanner, 3_000)).rejects.toThrow(
  ...
  it('gives each attempt the per-attempt bound, not the whole wait', async () => {
    ...
    const err = await failureOf(connect(peer.path, alive, noBanner, 2_500, 600))
  ...
  it('keeps the attempt bound INSIDE the deadline when the deadline is shorter', async () => {
    ...
    const started = Date.now()
    const err = await failureOf(connect(peer.path, alive, noBanner, 700, 30_000))
  ...
  it('FAILS within the deadline when nothing ever binds the path', async () => {
    const started = Date.now()
    await expect(connect(socketPath(), alive, noBanner, 500)).rejects.toThrow(
  ...
  it('NAMES THE CAUSE of an ordinary connect failure', async () => {
     ...
     */
    const err = await failureOf(connect(socketPath(), alive, noBanner, 500))
