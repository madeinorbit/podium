...
export const claudeCodeStateProvider: AgentStateProvider = {
  ...
  translate: (payload) => claudeCodeInstrumentation.payloadCodec.decode(payload),
...
function bootEventsForClaudeRecords(records: unknown[]): AgentStateEvent[] {
  const state = claudeHookClassifier().classify(records, 'default')
