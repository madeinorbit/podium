...
import { type CacheMutation, type AuthorityReadPort, type ReplicaCacheStore, ReplicaStoreCorruptError } from './ports'
...
export function validateFrame(frame: DeltaFrame): string | null {
	...
	for (const change of frame.changes) {
		...
		if (change.seq <= frame.fromSeq || change.seq > frame.seq) return 'change outside the covered range'
