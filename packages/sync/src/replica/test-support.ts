...
export function watermark(fromSeq: number, seq: number, overrides: Partial<DeltaFrame> = {}): DeltaFrame {
...
async function* failingStream(error = new Error('bootstrap stream failed')): AsyncIterable<BootstrapChunk> {
	await Promise.resolve()
	throw error
}
