 ...
 *
 * Lives in @podium/runtime (moved from apps/server) so that `podium setup` —
 * which usually runs before any server exists on the box — can probe a pasted
 * URL directly instead of going through a local server's router (POD-4534).
 * The wire protocol is unchanged; apps/server re-exports this module.
 */
...
import type { CheckResult } from './connect-check'
import {
...
export type { CheckError, CheckResult } from './connect-check'
  ...
  return {
    resolve: () => {
      ...
      try {
        ...
        // No identity on this box yet: there is nothing to look up, and a rescue path
        // must never throw at the caller that is already recovering from a failure.
        return Promise.resolve(undefined)
      }
      return resolveLocatorRecord({ baseUrl: base, installationId, fetch: fetchImpl, timeoutMs })
    },
    register: () =>
      attempt(() => {
        const identity = deps.identity()
