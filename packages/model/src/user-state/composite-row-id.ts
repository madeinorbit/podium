 ...
 * separately, written once for the fifth (PDM-408).
 *
 ...
 * `(eventId, subject)` and `issueMarks` is `(userId, issueId)`. Every one of
 * them joins on `\n` and escapes both the separator and the backslash, so a part
 * containing the separator cannot collide with a different pair.
 *
 * **WHY THIS IS SHARED AND NOT A FIFTH COPY.** For `issueMarks` the parse is
 * load-bearing in a way the others' are not: `feed-visibility.ts` decides WHO
 * RECEIVES the row by parsing the user back out of its id. A parser that
 * disagreed with its writer by a single escape would deliver one person's pins
 * and unread state to another — which is the exact defect PDM-402/PDM-408 exist
