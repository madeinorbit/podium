 ...
 *
 * Currently modelled: the Issue vocabulary (POD-791). Sessions and the remaining
 * entities follow under POD-302/POD-364-368; `packages/domain`'s pure predicates
 * are absorbed here by POD-299 (ADR 8 D4 records `packages/domain` →
 * `packages/model` as a rename+absorb, so this package is that rename's
 * destination, standing up ahead of it).
 */
export * from './fields'
export * from './ids'
export * from './issue'
