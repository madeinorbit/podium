...
const HANDOFF_ROWS: readonly MatrixRow[] = [
  {
    ...
    attribution: { actor: 'required', onBehalfOf: 'required', note: '`sourceMachineId` + `exportedAt` are export PROVENANCE — durable manifest facts, not replica-delivery facts, so they belong on the entity and not on the ReplicatedEnvelope.' },
    systemWriter: 'never-writes',
    inheritanceOnCreate: { kind: 'parent', from: ROW.sessionIdentity },
    visibilityMutability: {
      mutable: true,
      verbs: ['share', 'unshare', 'revoke', 'grant-use'],
      note: 'PHASE 2 MUST HANDLE: follows the session, plus target-machine `use`.',
    },
    open: [],
  },
]
...
const SYNC_ROWS: readonly MatrixRow[] = [
  ...
  },
  {
    ...
    owner: { kind: 'none', reason: 'substrate', note: 'Dedupe bookkeeping.' },
    visibility: 'deployment-substrate',
    grants: NO_GRANTS_SUBSTRATE,
    attribution: { actor: 'required', onBehalfOf: 'required' },
    systemWriter: 'may-write',
    ...
    visibilityMutability: { mutable: false, verbs: [], note: 'Never replicated.' },
    open: [],
  },
  {
    id: ROW.clientOutbox,
  ...
  },
  {
    ...
    visibilityMutability: {
      mutable: true,
      verbs: ['share', 'unshare', 'revoke'],
      note: 'PHASE 2 MUST HANDLE: the cursor is where a rescope LANDS. A scoped bootstrap reads the principal’s slice at `(feedId, epoch, seq)`; the shape of D6’s chunked bootstrap is unaffected.',
    },
    open: [],
  },
]
...
const MULTI_USER_ROWS: readonly MatrixRow[] = [
  ...
  },
  {
    ...
    owner: { kind: 'none', reason: 'secret', note: 'D15.' },
    visibility: 'secret',
    grants: NO_GRANTS_SECRET,
    attribution: { actor: 'required', onBehalfOf: 'required' },
    systemWriter: 'may-write',
