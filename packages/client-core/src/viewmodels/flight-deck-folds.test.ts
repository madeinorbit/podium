...
describe('flight deck fold migration and defaults', () => {
  it('keeps unresolved legacy IDs until an unfiltered resolved topology is supplied', () => {
    ...
    expect(first.get('leaf')).toBe('closed')
    ...
    expect(complete.get('branch')).toBe('open')
    const persisted = readFlightDeckFolds(writeFlightDeckFolds(complete))
    expect(persisted.get('branch')).toBe('open')
    expect(persisted.get('leaf')).toBe('closed')
    expect(persisted.get(deckFoldKey('branch', 'branch'))).toBe('open')
    expect(flightDeckBranchFolded(row('branch', ['child']), persisted)).toBe(false)
  })
  ...
  it('does not reinterpret a placed leaf fold after a later child arrives', () => {
    ...
    expect(migrated.get('leaf')).toBe('closed')
  ...
  it('can place a branch from a recorded child without declaring any leaf resolved', () => {
    ...
    expect(next.get('branch')).toBe('closed')
  ...
  it('opens current exceptions by default while honoring explicit closure', () => {
    ...
    const taskRequestBase = row('task-request', [], [session('idle', 'idle')])
    const taskRequest = { ...taskRequestBase, issue: { ...taskRequestBase.issue, needsHuman: true } } as FlightDeckRow
    ...
    expect(flightDeckRosterFolded(taskRequest, new Map())).toBe(false)
    expect(flightDeckRosterFolded(closed, new Map([[deckFoldKey('roster', 'closed'), 'closed']]))).toBe(true)
    expect(flightDeckRosterFolded(taskRequest, new Map([[deckFoldKey('roster', 'task-request'), 'closed']]))).toBe(true)
  })
})
