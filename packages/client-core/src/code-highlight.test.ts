...
describe('shared code tokens', () => {
  it.each(['ts', 'tsx', 'js', 'sh', 'jsonc', 'py', 'sql', 'yml', 'rs', 'md', 'html'])('preserves exact source for %s', lang => {
    const source = 'const x = "<script>&雪";\n\n'
    expect(highlightCode(source, lang).map(token => token.text).join('')).toBe(source)
  })
  ...
  it('detects a strong unlabelled code sample', () => {
    expect(highlightCode('def greet(name):\n    # greeting\n    print("hello")\n    return None').some(token => token.scope)).toBe(true)
  })
  it('returns synchronous semantic scopes with dotted names intact', () => {
    const tokens = highlightCode('class Child extends Parent {}\nfunction greet() { return true }', 'ts')
