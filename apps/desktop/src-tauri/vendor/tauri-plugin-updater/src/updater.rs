...
impl Updater {
    pub async fn check(&self) -> Result<Option<Update>> {
        ...
        // Preserve feed discovery semantics, including platform validation when not newer.
        let update = self.update_from_release(raw_json.unwrap())?;
        Ok(should_update.then_some(update))
    }

    /// Construct an installer for an already-authorized immutable release, without
    /// consulting endpoints or applying the discovery version comparator.
    ///
    /// This does not authorize installation: the caller must retain its durable
    /// approval/admission checks. Download still verifies with this updater's
    /// configured public key, and uses its existing installer and transport settings.
    pub fn update_from_release(&self, raw_json: serde_json::Value) -> Result<Update> {
...
mod tests {
    ...
    fn configured_updater() -> Updater {
        Updater {
            ...
            app_name: "Podium".into(),
            current_version: Version::parse("2.0.0").unwrap(),
            version_comparator: Some(Arc::new(|_, _| panic!("recovery consulted discovery"))),
            timeout: None,
            proxy: Some(Url::parse("https://proxy.invalid").unwrap()),
            no_proxy: false,
            endpoints: vec![Url::parse("https://feed.invalid").unwrap()],
            arch: "x86_64",
            target: Some("native".into()),
            headers: HeaderMap::new(),
            extract_path: PathBuf::from("approved-install-path"),
            on_before_exit: Some(Arc::new(|| {})),
    ...
    fn authorized_release_keeps_verifier_without_discovery() {
        ...
        for version in ["1.0.0", "2.0.0", "3.0.0"] {
            let release = serde_json::json!({
                "version": version,
                "url": "https://immutable.invalid/approved-artifact",
                "signature": "approved-signature",
            });
            let update = updater.update_from_release(release.clone()).unwrap();
            assert_eq!(update.version, version);
            assert_eq!(update.current_version, "2.0.0");
            assert_eq!(
                update.download_url.as_str(),
                release["url"].as_str().unwrap()
            );
            assert_eq!(update.signature, "approved-signature");
            assert_eq!(update.config.pubkey, "configured-key");
            assert!(!update.config.dangerous_accept_invalid_certs);
            assert!(!update.config.dangerous_accept_invalid_hostnames);
            assert_eq!(update.extract_path, updater.extract_path);
            assert_eq!(update.proxy, updater.proxy);
            assert_eq!(update.installer_args, updater.installer_args);
            assert_eq!(update.current_exe_args, updater.current_exe_args);
            assert!(Arc::ptr_eq(
                update.on_before_exit.as_ref().unwrap(),
                updater.on_before_exit.as_ref().unwrap()
            ));
            assert!(Arc::ptr_eq(
                update.configure_client.as_ref().unwrap(),
                updater.configure_client.as_ref().unwrap()
            ));
            assert_eq!(update.raw_json, release);
        }
        assert!(updater
            .update_from_release(serde_json::json!({
                "version": "not-semver", "url": "https://immutable.invalid/a", "signature": "s"
            }))
            .is_err());
        assert!(updater
            .update_from_release(serde_json::json!({
