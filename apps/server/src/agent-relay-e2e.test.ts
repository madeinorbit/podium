 ...
 *       → startAgentRelayServer loopback HTTP           (daemon/agent-relay)
 *         → hub.relay → send(agentRelayRequest)         (daemon/agent-relay)
 *           → registry.onDaemonMessageFrom              (server WS dispatch, REAL)
 *             → registry.runAgentRelay                  (allowlist + capability mint + P1a gate)
 *               → makeIssueCaller → appRouter.createCaller (capability-scoped tRPC caller, REAL)
 *           ← toMachine(agentRelayResult)               (server → daemon channel, REAL)
 ...
 */
describe('agent relay end-to-end (CLI → daemon relay → server capability gate → back)', () => {
  const machineId = 'm1'
  const repoPath = '/repo'
  let registry: SessionRegistry
  let relayServer: Awaited<ReturnType<typeof startAgentRelayServer>>
  ...
  beforeAll(async () => {
    ...
    overrideClient = makeRelayIssueClient(relayServer.endpointFor(asSessionId(sA)), {
      outsideScope: true,
    })
  })
