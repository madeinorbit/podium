...
describe('MobilePairingManager', () => {
  it('binds approval to the minting user and completes exactly once', () => {
    ...
    const claim = pairing.claim({ ...claimInput, pairCode: grant.pairCode }, 101)
    expect(claim).toEqual({
      claimId: 'token-3',
      phrase: ['amberanchor', 'amberanchor', 'amberanchor'],
      expiresAt: new Date(1_100).toISOString(),
    })
    expect(pairing.claim({ ...claimInput, pairCode: grant.pairCode }, 102)).toBeUndefined()
    ...
    expect(pairing.complete(claim!.claimId, SECRET.toString('base64url'), 105)).toBeUndefined()
  })
  ...
  it('destroys a claim after too many wrong completion secrets', () => {
    ...
    expect(pairing.complete(claim.claimId, wrong, 102)).toBeUndefined()
    expect(pairing.complete(claim.claimId, wrong, 103)).toBeUndefined()
    expect(pairing.status(grant.pairingId, FIRST_ADMIN_USER_ID, 104)).toEqual({
      state: 'denied',
    })
    expect(pairing.complete(claim.claimId, SECRET.toString('base64url'), 104)).toBeUndefined()
  })
  ...
  it('denies without minting and does not expose wrong-kind daemon grants', () => {
    const pairing = manager()
    const grant = pairing.mint(FIRST_ADMIN_USER_ID, 100)
    pairing.claim({ ...claimInput, pairCode: grant.pairCode }, 101)
    ...
    expect(pairing.complete('token-3', SECRET.toString('base64url'), 103)).toBeUndefined()
    expect(pairing.claim({ ...claimInput, pairCode: 'AB12-CD34' }, 103)).toBeUndefined()
  })
})
