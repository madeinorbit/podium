...
export const REACTIONS = [
  ...
  },
  {
    ...
    description: 'Recapture session machine names and adopt local placeholders after fleet changes.',
    trigger: 'machine.connected',
    durability: 'durable',
    replay: {
      mode: 'startup-reconcile',
      sourceOfTruth: 'machines table plus durable session rows',
      reauthorizeAtApply: true,
    },
    idempotency: { key: 'machineId + sessionId + machine row revision', duplicatePolicy: 'coalesce' },
    ordering: 'Per-machine EventBus order; session publication remains ledger ordered.',
    retry: 'Boot reconciliation repairs a missed connect/disconnect recapture.',
    failureOwner: 'sessions derived-field maintainer',
    observability: { registry: true, events: ['machine.connected', 'machine.disconnected'], metrics: [] },
    principal: system(),
    scopeInvariant: 'Updated session rows retain their existing owner and visibility.',
  },
  {
    ...
    trigger: 'settings.changed',
    durability: 'in-memory',
    replay: { mode: 'none', reason: 'Live session state is re-seeded and settings are read at boot.' },
  ...
  },
  {
    ...
    observability: { registry: true, events: ['issue.mailSent'], metrics: ['message delivery attempts'] },
    principal: delegated(),
  ...
  },
  {
    ...
    failureOwner: 'message delivery service',
    observability: { registry: true, events: ['oplog.appended'], metrics: ['message delivery attempts'] },
    principal: delegated(),
  ...
  },
  {
    ...
    failureOwner: 'message delivery service',
    observability: { registry: true, events: ['session.stateChanged'], metrics: ['message delivery attempts'] },
    principal: delegated(),
  ...
  },
  {
    ...
    failureOwner: 'message delivery service',
    observability: { registry: true, events: ['transcript.delta'], metrics: ['message confirmations'] },
    principal: delegated(),
  ...
  },
  {
    ...
    idempotency: { key: 'lock name + holder session id + lease generation', duplicatePolicy: 'safe-repeat' },
  ...
  },
  {
    ...
    idempotency: { key: 'machineId + native conversation id + source revision', duplicatePolicy: 'deduplicate' },
    ...
    observability: { registry: true, events: ['conversations.changed'], metrics: ['mirror/index diagnostics'] },
    principal: system(),
  ...
  },
  {
    ...
    durability: 'in-memory',
    replay: { mode: 'none', reason: 'Completed turns are not replayed to external chat after restart.' },
    idempotency: { key: 'owner user + thread id + completed turn id', duplicatePolicy: 'deduplicate' },
    ...
    observability: { registry: true, events: ['superagent.turnEnded'], metrics: ['Telegram send failures'] },
    principal: delegated(),
    scopeInvariant: 'Outbound route is resolved from the thread owner; inbound identity depends on POD-1080 and unknown chats fail closed.',
  },
  {
    ...
    trigger: 'settings.changed',
    durability: 'in-memory',
    replay: { mode: 'none', reason: 'Startup configures adapters from current settings and binding rows.' },
    idempotency: { key: 'user + bot-token fingerprint + route revision', duplicatePolicy: 'safe-repeat' },
    ...
    failureOwner: 'Telegram messaging bridge',
    observability: { registry: true, events: ['settings.changed'], metrics: ['Telegram configuration failures'] },
    principal: system(),
    scopeInvariant: 'Configuration may read server secrets but never exposes them or invents a user binding.',
  },
  {
    ...
    trigger: 'session.stateChanged',
    durability: 'in-memory',
    replay: { mode: 'none', reason: 'Typing presence is transient and is cleared on process restart.' },
    ...
    failureOwner: 'Telegram messaging bridge',
    observability: { registry: true, events: ['session.stateChanged', 'session.exited'], metrics: ['typing send failures'] },
    principal: delegated(),
  ...
  },
  {
    ...
    failureOwner: 'Telegram messaging bridge',
    observability: { registry: true, events: ['superagent.turnEnded'], metrics: ['Telegram recap failures'] },
    principal: delegated(),
    scopeInvariant: 'Transcript source and destination must have the same owner user.',
  },
  {
    id: 'automations.scheduled-runs',
    description: 'Materialize due durable automation occurrences as delegated sessions and run rows.',
    ...
    ordering: 'Chronological occurrence order per automation; ledger orders resulting rows globally.',
    retry: 'Janitor retries unmaterialized occurrences; terminal failures are recorded as run outcomes.',
    failureOwner: 'automation scheduler/service',
    observability: { registry: true, events: ['automation run rows'], metrics: ['automation outcomes'] },
    principal: delegated(),
    scopeInvariant: 'Creator delegation is resolved live on every fire/replay; revoked creators cannot run.',
  },
  {
    ...
    observability: { registry: true, events: ['steward event cursor'], metrics: ['steward delivery failures'] },
    principal: system(),
  ...
  },
  {
    ...
    observability: { registry: true, events: ['maintenance run records'], metrics: ['prune/expiry duration'] },
    principal: system(),
    scopeInvariant: 'System expiry never changes ownership or attributes a human actor.',
  },
  {
    id: 'startup.boot-reconcile',
    description: 'Reconcile durable issues, sessions, messages, conversations, and derived fields at boot.',
    trigger: 'startup',
    durability: 'durable',
    replay: {
      mode: 'startup-reconcile',
    ...
    idempotency: { key: 'entity kind + entity id + durable revision', duplicatePolicy: 'safe-repeat' },
    ...
    observability: { registry: true, events: ['boot diagnostics'], metrics: ['reconcile duration/failures'] },
    principal: system(),
    scopeInvariant: 'Boot writes are system-attributed, preserve entity scope, and never stamp a human.',
  },
