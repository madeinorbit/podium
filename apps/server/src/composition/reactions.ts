...
export const REACTIONS = [
  ...
  },
  {
    ...
    replay: {
      mode: 'none',
      reason: 'Client bootstrap reads the durable authority cursor after restart.',
    },
    idempotency: { key: 'feed sequence', duplicatePolicy: 'coalesce' },
    ordering: 'Global ledger sequence; cursors only advance monotonically.',
    retry: 'The next publication advances to an equal or newer sequence.',
    failureOwner: 'session publication coordinator',
    observability: { registry: true, events: ['feed.published'], metrics: [] },
    principal: system(),
