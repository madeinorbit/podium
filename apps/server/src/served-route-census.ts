...
export const RAW_ROUTE_POLICIES: readonly RawRoutePolicy[] = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    rationale:
      "Dispatches into the superagent's tool belt. Its issue_* tools bridge the ISSUE COMMAND REGISTRY — the same definitions, carrying the same `authz`, that the `issues` tRPC family is excluded from the projection census for — and that registry's totality is kept by `modules/issues/registry.ts` and `classification-totality.test.ts`. The production builder enables that bridge with `issueBelt: true`; the own population is the hand-written specs emitted before it in `modules/superagent/tools.ts`, witnessed with `issueBelt: false` and `searchIndexEnabled: true`. Bounded own-tool read inventory: `list_sessions` calls `sessions.listSessions(undefined, 'listAllTool')` and reaches `SessionView.project` → `SessionStateService.visibleSessions` with `INTERNAL_PROJECTION_READ`; the SessionListCaller union labels this call; listAllTool is a performance caller label, not a principal or authorization scope. Its `issues.issueForCwd`/`issues.getMeta` enrichment is a separate issue-metadata join, not part of that session projection; this inventory does not establish a separate authorization consumer for that join. `read_session_transcript` and `answer_question` pass `memoryReader` to `machines/rpc.ts`; its `memory.canReadSession` invokes `MemoryVisibilityPolicy.mayReadSession` through `memory/visibility.ts` before daemon/lake access. `search_conversations` and `search_all` pass the same reader to `MemorySearchService` and its `MemoryVisibilityPolicy` checks. `recap_session` is different: it passes a `ReaderRef` to `SessionReadToolkit.recap` (`read-toolkit.ts`) for event logging and per-reader watermarks; `resolveVisible` uses the relay-bound `sessionById`/internal projection, and relay.ts binds its `readTranscript` dependency to a system-bound transcript reader before `machines/rpc.ts`. Thus the `ReaderRef` is not a transcript authorization principal, and caller-specific ownership for that direct path is not established here. Repository/git reads, control-tool existence reads, and indirect reads outside this bounded inventory remain unresolved; this row makes no global safety guarantee or leak finding.",
  },
  {
    method: 'GET',
    path: '/mcp',
