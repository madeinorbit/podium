...
describe('the manifest and the migration folders agree', () => {
  it('contains this migration exactly once, and no name is duplicated', () => {
    ...
    // This used to assert the migration was LAST. That held when this issue
    // branched and is not a property worth pinning: siblings land migrations
    // concurrently (POD-306's `add-feed-identity-table` now sorts after this
    // one), and the runner is pure set membership over names rather than a
    // positional replay. Pinning "last" makes the test a function of merge
    // ORDER — it fails for whichever of two correct branches lands second.
    //
    // What actually matters for a name-keyed runner is that no name repeats,
    // so that is asserted directly and over EVERY entry rather than this one.
    const names = DRIZZLE_MIGRATIONS.map((m) => m.name)
