...
export const workflowEvents = sqliteTable(
  'workflow_events',
  {
    ...
    // ADR 9 D5 A3 (POD-731): attribution is a PAIR. `actor_*` above is WHICH
    // agent or session acted; this is WHICH HUMAN it acted for, stamped from the
    // transport principal and never from payload. Nullable because every row
    // written before this column existed has no human recorded and inventing one
    // would be a lie in an audit trail — and because a `system` principal has no
    // human by construction (D14.2: never assigned one).
