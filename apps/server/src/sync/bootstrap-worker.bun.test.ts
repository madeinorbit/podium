...
const { Authority, DEVICE_GRADE_PRINCIPAL, GrantEdgeVisibilityPolicy, NoDelegationsGranted } = await import('@podium/sync')
...
interface TestServer { port: number; stop(force: boolean): void | Promise<void> }
const runtime = globalThis as typeof globalThis & {
  Bun: { serve(options: { hostname: string; port: number;
    fetch(request: Request, server: { upgrade(request: Request): boolean }): Response | Promise<Response> | undefined;
...
async function fixture() {
  ...
  registerSyncRoutes(app, { authority, worker: () => worker,
    ...
    serving: { identity: async () => ({ feedId: 'fixture-feed', epoch: 'fixture-epoch' }), retentionFloor: async () => (await store.sync.minChangeSeq()) ?? 1 },
  })
...
it('retains worker completion metrics after client cancellation releases the body', async () => {
  const f = await fixture()
  try {
    await f.store.sync.appendChanges([{ entity: 'repo', entityId: '/r', op: 'upsert', payload: JSON.stringify({ text: 'x'.repeat(1024 * 1024) }) }], 1)
    const abort = new AbortController()
    const transfer = f.worker.bootstrap({ transferId: 'cancel-metrics', principal, feedId: 'f', epoch: 'e', encoding: 'identity' }, abort.signal)
    await transfer.meta
...
it('50 MiB slow bootstrap keeps same-server health and websocket ping p95 below 100 ms', async () => {
  ...
  try {
    ...
    server = runtime.Bun.serve({ hostname: '127.0.0.1', port: 0,
