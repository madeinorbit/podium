...
describe('POD-1646 — resolving a principal does not project every session', () => {
  it('projects nothing at all: the chain walk is a by-id read', async () => {
    const reg = await SessionRegistry.create(undefined, undefined, { instanceId: 'default' })
    const CORPUS = 20
    const leaf = await seedChain(reg, CORPUS, 3)
  ...
  it('costs the same whether the machine holds 8 sessions or 64', async () => {
    ...
    const smallLeaf = await seedChain(small, 8, 3)
    const large = await SessionRegistry.create(undefined, undefined, { instanceId: 'default' })
    const largeLeaf = await seedChain(large, 64, 3)
