  ...
  const bag = life as any
    ...
    bag.workspace = new SessionWorkspace({
      ...
      onWorktreesChanged: (repoPath, machineId) =>
        bag.deps.onWorktreesChanged(repoPath, machineId),
    })
    ...
    bag.clientControl = new SessionClientControl({
      ...
      setDraft: (principal, clientId, sessionId, text) => {
        bag.sessionStateEnvelope().execute(
          'sessions.setDraft',
    ...
    bag.autoContinue = new AutoContinueController({
      ...
      isEnabled: () =>
        bag.store.settings.getSettingsFor(bag.settingsViewer()).autoContinue.enabled,
