...
describe('oracle: pins', () => {
  ...
  it(`${MUST_NOT_CHANGE}: a pin is keyed (userId, kind, id) — the writer sees it and another user's slice does not`, async () => {
    ...
    expect(await o.store.sessions.listPins(asUserId('user:somebody-else'))).toEqual({
      panels: [],
      worktrees: [],
      repos: [],
    })
...
describe('oracle: tab order', () => {
  ...
  it(`${MUST_NOT_CHANGE}: tab order is keyed (userId, worktree) — the writer sees it and another user's slice does not`, async () => {
    ...
    expect(await o.store.sessions.listTabOrders(asUserId('user:somebody-else'))).toEqual({})
  })
...
describe('oracle: the wake fence (POD-1472)', () => {
  it(`${MUST_NOT_CHANGE}: a resurrect spawn frame carries the fence the wake allocated — generation, binding version and provider session id`, async () => {
    ...
    const durable = await o.store.observationCheckpoints.get(sessionId)
  ...
  it(`${MUST_NOT_CHANGE}: resuming a PARKED conversation by resume ref wakes the existing row — same id, one row, and a fresh fence on its frame [POD-1473]`, async () => {
    ...
    expect(spawnFrames(o.daemon, sessionId)).toEqual([
      expect.objectContaining({
        ...
        sessionId,
        resume: RESUME,
        observationGeneration: (await o.store.observationCheckpoints.get(sessionId))
