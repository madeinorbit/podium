...
- DECOMPOSE multi-task messages: when one message contains several distinct tasks, file one issue per task; merge tasks into one issue only when they touch the same component or files. When the user says go, start the non-conflicting issues in parallel (one issue_start each, own worktree per issue) — never funnel them into a single session; issues touching the same files get a blocks-dependency and run in sequence.
