...
describe('SpecsService (modules/specs, pspec #135)', () => {
  it('gates every proc on the repo-root allowlist', async () => {
    const repo = tmpRepo()
    const svc = new SpecsService({ repoRoots: () => [repo] })
    expect((await svc.list({ repoPath: repo })).some((c) => c.id === 'SP-root')).toBe(true)
    await expect(svc.list({ repoPath: '/not/registered' })).rejects.toThrow(/known repository/)
    await expect(svc.save({ repoPath: '/not/registered', id: 'SP-root', body: 'x' })).rejects.toThrow(
      /known repository/,
    )
  })

  it('create/save/get/search/remove round-trip through the file store', async () => {
    const repo = tmpRepo()
    const svc = new SpecsService({ repoRoots: () => [repo] })
    const created = await svc.create({ repoPath: repo, title: 'Retry rules', parent: 'SP-root' })
    await svc.save({ repoPath: repo, id: created.id, body: '<p>retry at most twice</p>' })
    expect((await svc.get({ repoPath: repo, id: created.id }))?.body).toContain('twice')
    expect((await svc.search({ repoPath: repo, query: 'twice' })).map((h) => h.id)).toContain(created.id)
    await svc.remove({ repoPath: repo, id: created.id })
    expect(await svc.get({ repoPath: repo, id: created.id })).toBeNull()
  })

  it('a REGISTERED root that does not exist on this host is a typed error, not a 500', async () => {
  ...
  it('pspec validation failures surface as BAD_REQUEST, not 500', async () => {
    const repo = tmpRepo()
    const svc = new SpecsService({ repoRoots: () => [repo] })
    let caught: unknown
    try {
      await svc.save({ repoPath: repo, id: 'SP-zzzz', body: '<p>x</p>' }) // unknown component
    } catch (err) {
      caught = err
    }
    expect(caught).toBeInstanceOf(TRPCError)
