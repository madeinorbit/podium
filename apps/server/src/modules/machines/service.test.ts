...
describe('MachinesService authority epoch and the committed-write ordering', () => {
  ...
  // THE FUNNEL CHECK MOVED OUT, to `apps/server/src/store-authority-write-funnel.test.ts`.
  //
  // Not tidiness: a test that READS repository source must carry repo-root path
  // literals so `scripts/server-test-shards.ts` can put those files in the
  // shard's Turbo inputs, and doing that pins the WHOLE FILE to the `boundary`
  // shard. Leaving it here would have dragged sixty behavioural tests out of
  // `services` with it -- and leaving it here WITHOUT the literals, which is what
  // the first revision did, meant the scanned repositories were not in this
  // lane's cache key at all: a commit adding a raw write would have replayed as a
  // cache hit. The ordering witness above stays; it reads no files.
})
