 ...
 *  row half — so the label list is empty unless a test is about labels. */
...
describe('issueRowToProjection [POD-796]', () => {
  it('carries the durable row through, revision included', () => {
    ...
    expect(p).toMatchObject({
      ...
      // and `draft` are the issue's INTENT, and the session entity uses both
      // words for different things — so the projection spells them
      // `intentOrigin` / `isDraftVessel`. The storage column keeps the old name;
      // the adapter is where the two meet.
      intentOrigin: 'agent',
      audience: 'human',
      isDraftVessel: false,
      archived: false,
  ...
  it('decodes the panel JSON column into the structured value', () => {
    ...
    expect(issueRowToProjection(row(), NO_LABELS).panel).toEqual({
  ...
  it('carries NO sessions, NO memberSessionIds and NO derived rollups', () => {
    ...
    // smaller print.
    const p = issueRowToProjection(row(), NO_LABELS) as Record<string, unknown>
    for (const derived of [
      // Per-user state, absent from the projection BY CONSTRUCTION on this
      // branch (model `aggregates/issue.ts`) rather than merely unpopulated:
      // `readAt` and `unread` belong to a reader, not to the issue's own row.
      'readAt',
      'sessions',
      'memberSessionIds',
      'sessionSummary',
      ...
      'displayRef',
      'prefix',
      // NOT 'labels': on this branch labels are a durable member of the
      // aggregate and arrive as `issueRowToProjection`'s second argument, so the
      // projection carries them. They are still not DERIVED from another entity,
  ...
  it('THROWS on a row with no revision rather than fabricating one', () => {
    ...
    expect(() => issueRowToProjection(noRevision, NO_LABELS)).toThrow(/no revision/)
  })
  ...
  it('REFUSES a stored enum value the model does not understand', () => {
    ...
    expect(() => issueRowToProjection(row({ stage: 'not-a-stage' }), NO_LABELS)).toThrow()
  })
})

describe('issueProjectionRows: all-or-nothing [POD-796]', () => {
  it('projects every row when they all project', () => {
    const rows = issueProjectionRows([row({ id: 'iss_1' }), row({ id: 'iss_2' })], () => NO_LABELS)
