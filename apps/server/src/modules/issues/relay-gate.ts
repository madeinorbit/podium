...
/** The capability-scoped caller factory (server.ts injects it as registry.makeIssueCaller). */
...
export interface IssueRelayGateDeps {
  /** Read the injectable factory LIVE — tests/server.ts assign it after construction. */
  makeIssueCaller(): IssueCallerFactory | undefined
...
export class IssueRelayGate {
  ...
  async run(
    ...
    try {
      ...
      const make = this.deps.makeIssueCaller()
      if (!make) {
        reply({ ok: false, error: 'issue relay is not configured' })
        return
      }
      const caller = make(this.deps.capabilityForSession(msg.sessionId), msg.outsideScope)
