...
describe('machines store', () => {
  ...
  it('repos table is re-keyed to (machine_id, path) with origin_url', () => {
    ...
    s.removeRepo('/home/u/a')
    expect(s.listRepos().map((r) => r.path)).toEqual(['/home/u/b'])
    s.close()
  })
  ...
  it('listRepoPaths returns a flat string[] for back-compat', () => {
    const s = new SessionStore(':memory:')
    s.addRepo('/abs/one')
    s.addRepo('/abs/two', 'm2')
    const paths = s.listRepoPaths()
  ...
  it('listRepos(machineId) filters to one machine', () => {
    const s = new SessionStore(':memory:')
    s.addRepo('/abs/local')
    s.addRepo('/abs/remote', 'm2')
    expect(s.listRepos('__local__').map((r) => r.path)).toEqual(['/abs/local'])
    expect(s.listRepos('m2').map((r) => r.path)).toEqual(['/abs/remote'])
    expect(s.listRepoPaths('m2')).toEqual(['/abs/remote'])
    s.close()
  })

  it('getMachine returns a record when it exists', () => {
    const s = new SessionStore(':memory:')
    s.upsertMachine({ id: 'm2', name: 'server', hostname: 'srv', tokenHash: hash('tok') })
    const m = s.getMachine('m2')
    expect(m?.id).toBe('m2')
    expect(m?.name).toBe('server')
    expect(s.getMachine('no-such')).toBeUndefined()
    s.close()
  })

  it('touchMachine updates last_seen_at and hostname', () => {
    const s = new SessionStore(':memory:')
    s.upsertMachine({ id: 'm3', name: 'box', hostname: 'old-host', tokenHash: hash('t') })
    s.touchMachine('m3', 'new-host')
    const m = s.getMachine('m3')
  ...
  it('multi-machine migration is idempotent — re-opening the same file db is a no-op', async () => {
    ...
    s1.upsertMachine({ id: 'm1', name: 'a', hostname: 'h', tokenHash: 'x' })
    s1.addRepo('/a')
    s1.upsertSession({
      id: 's1',
      agentKind: 'shell',
    ...
    expect(s2.loadSessions()[0]?.machineId).toBe('__local__')
    expect(s2.listMachines()).toHaveLength(1)
    expect(s2.listRepoPaths()).toEqual(['/a'])
    // The settings row written through migrate() survives the reopen — a proxy that
    // the meta table wasn't wiped.
    expect(s2.getSettings().sessionDefaults.agent).toBe('auto') // defaults always present
    s2.close()
    rmSync(file, { force: true })
  })
})
