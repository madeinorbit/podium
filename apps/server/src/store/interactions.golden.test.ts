...
it('collapses a duplicate open ask onto the existing row and says it did not insert', async () => {
  ...
  try {
    const first = await store.interactions.insert(ask())
    ...
    const second = await store.interactions.insert(ask({ id: 'int-2' }))
    expect(second.inserted).toBe(false)
    expect(second.row.id).toBe('int-1')
    expect(await store.interactions.get('int-2')).toBeNull()

    // A different session asking the same question is a different ask.
    const elsewhere = await store.interactions.insert(
...
it('re-asks after the first was resolved, because the dedupe index covers open rows only', async () => {
  const store = await openTestStore(':memory:')
  try {
    await store.interactions.insert(ask())
    expect(
      await store.interactions.answer({
        id: 'int-1',
        answer,
        answeredBy: 'human',
        deliveredVia: 'menu',
    ...
    const again = await store.interactions.insert(ask({ id: 'int-2' }))
...
it('answers exactly once, and records delivery only after the row was claimed', async () => {
  ...
  try {
    ...
    expect(await store.interactions.recordDelivery('int-1', 'structured')).toBe(false)
    ...
    expect(await store.interactions.recordDelivery('int-1', 'structured')).toBe(true)
    const row = await store.interactions.get('int-1')
...
it('reopens only the class of answer it was aimed at, and clears every trace of that answer', async () => {
  ...
  try {
    ...
    expect(await store.interactions.reopen('int-1', 'human')).toBe(false)
    expect((await store.interactions.get('int-1'))?.status).toBe('answered')

    expect(await store.interactions.reopen('int-1', 'policy')).toBe(true)
    const row = await store.interactions.get('int-1')
    ...
    expect((await store.interactions.listOpen(session)).map((r) => r.id)).toEqual(['int-1'])
...
it('retires a claimed row that close cannot reach, and close reaches the open one that retire cannot', async () => {
  const store = await openTestStore(':memory:')
  try {
    await store.interactions.insert(ask({ id: 'claimed', fingerprint: 'fp-claimed' }))
    await store.interactions.insert(ask({ id: 'open', fingerprint: 'fp-open' }))
    ...
    expect(await store.interactions.close('claimed', 'expired', at)).toBe(false)
    expect(await store.interactions.retireClaimed('open', 'expired', at, 'policy')).toBe(false)

    // And retire is guarded on who claimed it, exactly like reopen.
    expect(await store.interactions.retireClaimed('claimed', 'expired', at, 'human')).toBe(false)
    expect(await store.interactions.retireClaimed('claimed', 'expired', at, 'policy')).toBe(true)
    expect(await store.interactions.get('claimed')).toMatchObject({ status: 'expired', expiredAt: at })

    expect(await store.interactions.close('open', 'superseded', at)).toBe(true)
    expect(await store.interactions.get('open')).toMatchObject({ status: 'superseded', expiredAt: at })
...
it('closes every open ask on one session at once and returns exactly the ids that moved', async () => {
  ...
  try {
    ...
    await store.interactions.answer({ id: 'b', answer, answeredBy: 'human', deliveredVia: 'menu', at })
    ...
    expect(await store.interactions.closeSession(session, 'expired', at)).toEqual(['a', 'c'])
    expect(await store.interactions.get('a')).toMatchObject({ status: 'expired', expiredAt: at })
    expect(await store.interactions.get('c')).toMatchObject({ status: 'expired', expiredAt: at })
    expect((await store.interactions.get('b'))?.status).toBe('answered')
    // Another session's open ask is untouched.
    expect((await store.interactions.get('other'))?.status).toBe('asked')
...
it('enumerates open asks oldest first and a session’s history newest first', async () => {
  ...
  try {
    ...
    expect((await store.interactions.listOpen(session)).map((r) => r.id)).toEqual(['b', 'a', 'c'])
    // No session argument means every session's open asks.
    expect((await store.interactions.listOpen()).map((r) => r.id)).toEqual(['z', 'b', 'a', 'c'])
...
it('trims resolved rows by age and never trims an open ask, however old', async () => {
  ...
  try {
    ...
    await store.interactions.insert(ask({ id: 'ancient-open', fingerprint: 'fp-1', askedAt: old }))
    await store.interactions.insert(ask({ id: 'answered-old', fingerprint: 'fp-2', askedAt: old }))
    await store.interactions.insert(ask({ id: 'closed-old', fingerprint: 'fp-3', askedAt: old }))
    await store.interactions.insert(ask({ id: 'answered-new', fingerprint: 'fp-4', askedAt: old }))
    await store.interactions.answer({
      id: 'answered-old',
      answer,
      answeredBy: 'human',
    ...
    await store.interactions.close('closed-old', 'expired', '2026-02-01T00:00:00.000Z')
    await store.interactions.answer({
      id: 'answered-new',
      answer,
      answeredBy: 'human',
    ...
    expect(await store.interactions.pruneResolvedBefore('2026-03-01T00:00:00.000Z')).toBe(2)
    expect(await store.interactions.get('answered-old')).toBeNull()
    expect(await store.interactions.get('closed-old')).toBeNull()
    expect(await store.interactions.get('answered-new')).not.toBeNull()
