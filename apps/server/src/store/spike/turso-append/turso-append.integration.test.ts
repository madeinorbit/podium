...
describe.skipIf(SQLD === undefined)('change-log append over libsql', () => {
  it('assigns contiguous seqs across every chunk boundary', async () => {
    await withSlice(async (slice) => {
      ...
      const seqs = await slice.withSession(async (s) => await appendChangesLiteral(s, slice.db, slice.tables, rows, 1_000))
  ...
  it('gives the batched form the same seqs as the literal one', async () => {
    ...
    const literal = await withSlice(async (slice) =>
      await slice.withSession(async (s) => await appendChangesLiteral(s, slice.db, slice.tables, upsertRows(250), 1_000)),
    )
    const batched = await withSlice(async (slice) =>
      await slice.withSession(async (s) => await appendChangesBatched(s, slice.db, slice.tables, upsertRows(250), 1_000)),
    )
  ...
  it('continues contiguously from the head on a later append', async () => {
    await withSlice(async (slice) => {
      await slice.withSession(async (s) => await appendChangesLiteral(s, slice.db, slice.tables, upsertRows(120, 'a'), 1_000))
  ...
  it('derives each range from the statement that wrote it, not from the database', async () => {
    ...
    try {
      const seqsA = await a.withSession(async (s) =>
        await appendChangesLiteral(s, a.db, a.tables, upsertRows(100, 'c'), 1_000),
      )
      const seqsB = await b.withSession(async (s) =>
        await appendChangesLiteral(s, b.db, b.tables, upsertRows(50, 'd'), 1_000),
      )
      const seqsA2 = await a.withSession(async (s) =>
        await appendChangesLiteral(s, a.db, a.tables, upsertRows(100, 'e'), 1_000),
      )
  ...
  it('rolls the whole append back when a chunk throws, INCLUDING the counter', async () => {
    await withSlice(async (slice) => {
      ...
      expect(await slice.withSession(async (s) => await changesSince(s, slice.db, slice.tables, 0))).toEqual([])
      expect(await slice.withSession(async (s) => await latestChangeStates(s, slice.db, slice.tables))).toEqual([])
      expect(await slice.withSession(async (s) => await minChangeSeq(s, slice.tables))).toBeNull()
      ...
      expect(await slice.withSession(async (s) => await maxChangeSeq(s, slice.tables))).toBe(0)
      const next = await slice.withSession(async (s) =>
        await appendChangesLiteral(s, slice.db, slice.tables, upsertRows(5, 'g'), 2_000),
      )
  ...
  it('treats a payload-less upsert as "not installed"', async () => {
    await withSlice(async (slice) => {
      ...
      expect(await slice.withSession(async (s) => await changesSince(s, slice.db, slice.tables, 0))).toHaveLength(2)
      expect(await slice.withSession(async (s) => await latestChangeStates(s, slice.db, slice.tables))).toEqual([])
    })
  }, 60_000)
  ...
  it('reads the head from sqlite_sequence, so it survives head-pruning', async () => {
    await withSlice(async (slice) => {
      await slice.withSession(async (s) => await appendChangesLiteral(s, slice.db, slice.tables, upsertRows(200), 1_000))
      ...
      expect(await slice.withSession(async (s) => await minChangeSeq(s, slice.tables))).toBeNull()
      expect(await slice.withSession(async (s) => await maxChangeSeq(s, slice.tables))).toBe(200)
      ...
      const next = await slice.withSession(async (s) =>
        await appendChangesLiteral(s, slice.db, slice.tables, upsertRows(3, 'p'), 2_000),
      )
  ...
  it('keeps the head after a PARTIAL prune too, where MAX has not yet fallen', async () => {
    await withSlice(async (slice) => {
      ...
      expect(await slice.withSession(async (s) => await minChangeSeq(s, slice.tables))).toBe(151)
      expect(await slice.withSession(async (s) => await maxChangeSeq(s, slice.tables))).toBe(200)
    })
  }, 60_000)
  ...
  it('continues contiguously after the server process restarts', async () => {
    ...
    try {
      ...
      let head = 0
      try {
        await first.withSession(async (s) => await appendChangesLiteral(s, first.db, first.tables, upsertRows(120), 1_000))
        head = await first.withSession(async (s) => await maxChangeSeq(s, first.tables))
      } finally {
        await first.close()
      }
      expect(head).toBe(120)
      ...
      try {
        ...
        expect(await second.withSession(async (s) => await maxChangeSeq(s, second.tables))).toBe(120)
        const seqs = await second.withSession(async (s) =>
          await appendChangesLiteral(s, second.db, second.tables, upsertRows(10, 'q'), 2_000),
        )
  ...
  it('grants a free lease, refuses a held one, and takes over an expired one', async () => {
    await withSlice(async (slice) => {
      ...
      const first = await slice.withSession(async (s) =>
        await acquireLock(
          s,
          slice.db, slice.tables,
      ...
      const refused = await slice.withSession(async (s) =>
        await acquireLock(
          s,
          slice.db, slice.tables,
      ...
      expect(await slice.withSession(async (s) => await readLock(s, slice.db, slice.tables, 'repo', 'test:heavy'))).toBe(
        'session-a',
      )

      // Past A's expiry: the lease is free and B takes it over in place.
      const takeover = await slice.withSession(async (s) =>
        await acquireLock(
          s,
          slice.db, slice.tables,
  ...
  it('costs a round trip for the decision the append never has to make', async () => {
    await withSlice(async (slice) => {
      ...
      await slice.withSession(async (s) => await acquireLock(s, slice.db, slice.tables, request, '2026-01-01T00:00:00Z'))
      const granted = slice.counted.roundTrips.reset()

      await slice.withSession(async (s) =>
        await acquireLock(s, slice.db, slice.tables, { ...request, holderSessionId: 'b' }, '2026-01-01T00:00:00Z'),
      )
  ...
  it('costs two driver calls per chunk when batched, against one per row when not', async () => {
    await withSlice(async (slice) => {
      slice.counted.roundTrips.reset()
      await slice.withSession(async (s) => await appendChangesLiteral(s, slice.db, slice.tables, upsertRows(100, 'r'), 1_000))
      const literal = slice.counted.roundTrips.reset()

      await slice.withSession(async (s) => await appendChangesBatched(s, slice.db, slice.tables, upsertRows(100, 's'), 1_000))
