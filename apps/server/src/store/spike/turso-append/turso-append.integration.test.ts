...
describe.skipIf(SQLD === undefined)('change-log append over libsql', () => {
  it('assigns contiguous seqs across every chunk boundary', async () => {
    await withSlice(async (slice) => {
      ...
      const seqs = await slice.withSession((s) => appendChangesLiteral(s, slice.db, rows, 1_000))
  ...
  it('gives the batched form the same seqs as the literal one', async () => {
    ...
    const literal = await withSlice((slice) =>
      slice.withSession((s) => appendChangesLiteral(s, slice.db, upsertRows(250), 1_000)),
    )
    const batched = await withSlice((slice) =>
      slice.withSession((s) => appendChangesBatched(s, slice.db, upsertRows(250), 1_000)),
    )
  ...
  it('continues contiguously from the head on a later append', async () => {
    await withSlice(async (slice) => {
      await slice.withSession((s) => appendChangesLiteral(s, slice.db, upsertRows(120, 'a'), 1_000))
  ...
  it('derives each range from the statement that wrote it, not from the database', async () => {
    ...
    try {
      const seqsA = await a.withSession((s) =>
        appendChangesLiteral(s, a.db, upsertRows(100, 'c'), 1_000),
      )
      const seqsB = await b.withSession((s) =>
        appendChangesLiteral(s, b.db, upsertRows(50, 'd'), 1_000),
      )
      const seqsA2 = await a.withSession((s) =>
        appendChangesLiteral(s, a.db, upsertRows(100, 'e'), 1_000),
      )
  ...
  it('rolls the whole append back when a chunk throws, INCLUDING the counter', async () => {
    await withSlice(async (slice) => {
      await expect(
        slice.withSession((s) =>
          appendChangesLiteral(s, slice.db, upsertRows(250, 'f'), 1_000, {
            afterChunk: (i) => {
      ...
      expect(await slice.withSession((s) => changesSince(s, slice.db, 0))).toEqual([])
      expect(await slice.withSession((s) => latestChangeStates(s, slice.db))).toEqual([])
      expect(await slice.withSession((s) => minChangeSeq(s))).toBeNull()
      ...
      expect(await slice.withSession((s) => maxChangeSeq(s))).toBe(0)
  ...
  it('treats a payload-less upsert as "not installed"', async () => {
    await withSlice(async (slice) => {
      ...
      expect(await slice.withSession((s) => changesSince(s, slice.db, 0))).toHaveLength(2)
      expect(await slice.withSession((s) => latestChangeStates(s, slice.db))).toEqual([])
    })
  }, 60_000)
  ...
  it('reads the head from sqlite_sequence, so it survives head-pruning', async () => {
    await withSlice(async (slice) => {
      await slice.withSession((s) => appendChangesLiteral(s, slice.db, upsertRows(200), 1_000))
      ...
      await slice.counted.client.execute('DELETE FROM changes')

      expect(await slice.withSession((s) => minChangeSeq(s))).toBeNull()
      expect(await slice.withSession((s) => maxChangeSeq(s))).toBe(200)
      ...
      const next = await slice.withSession((s) =>
        appendChangesLiteral(s, slice.db, upsertRows(3, 'p'), 2_000),
      )
  ...
  it('keeps the head after a PARTIAL prune too, where MAX has not yet fallen', async () => {
    await withSlice(async (slice) => {
      await slice.withSession((s) => appendChangesLiteral(s, slice.db, upsertRows(200), 1_000))
      await slice.counted.client.execute('DELETE FROM changes WHERE seq <= 150')
      expect(await slice.withSession((s) => minChangeSeq(s))).toBe(151)
      expect(await slice.withSession((s) => maxChangeSeq(s))).toBe(200)
    })
  }, 60_000)
  ...
  it('continues contiguously after the server process restarts', async () => {
    ...
    try {
      ...
      let head = 0
      try {
        await first.withSession((s) => appendChangesLiteral(s, first.db, upsertRows(120), 1_000))
        head = await first.withSession((s) => maxChangeSeq(s))
      } finally {
        await first.close()
      }
      expect(head).toBe(120)
      ...
      try {
        ...
        expect(await second.withSession((s) => maxChangeSeq(s))).toBe(120)
        const seqs = await second.withSession((s) =>
          appendChangesLiteral(s, second.db, upsertRows(10, 'q'), 2_000),
        )
      ...
      try {
        ...
        await expect(
          session.executeBatch([
            {
              sql: 'INSERT INTO changes (entity, entity_id, op, payload, event_time) VALUES (?, ?, ?, ?, ?)',
        ...
        const remaining = await session.execute({
          sql: 'SELECT COUNT(*) AS n FROM changes',
          params: [],
          method: 'get',
          intent: 'read',
        })
  ...
  it('grants a free lease, refuses a held one, and takes over an expired one', async () => {
    await withSlice(async (slice) => {
      ...
      expect(await slice.withSession((s) => readLock(s, slice.db, 'repo', 'test:heavy'))).toBe(
        'session-a',
      )
  ...
  it('costs a round trip for the decision the append never has to make', async () => {
    await withSlice(async (slice) => {
      ...
      await slice.withSession((s) => acquireLock(s, slice.db, request, '2026-01-01T00:00:00Z'))
      const granted = slice.counted.roundTrips.reset()

      await slice.withSession((s) =>
        acquireLock(s, slice.db, { ...request, holderSessionId: 'b' }, '2026-01-01T00:00:00Z'),
      )
  ...
  it('costs one round trip for the bootstrap read whatever its size', async () => {
    await withSlice(async (slice) => {
      await slice.withSession((s) => appendChangesLiteral(s, slice.db, upsertRows(250), 1_000))
      slice.counted.roundTrips.reset()
      const world = await slice.withSession((s) => latestChangeStates(s, slice.db))
  ...
  it('costs two driver calls per chunk when batched, against one per row when not', async () => {
    await withSlice(async (slice) => {
      slice.counted.roundTrips.reset()
      await slice.withSession((s) => appendChangesLiteral(s, slice.db, upsertRows(100, 'r'), 1_000))
      const literal = slice.counted.roundTrips.reset()

      await slice.withSession((s) => appendChangesBatched(s, slice.db, upsertRows(100, 's'), 1_000))
