...
const JSON_COLUMNS: readonly OracleEntry[] = [
  ...
  },
  {
    ...
    where: 'session_id = ?',
    params: (f) => [f.sessionId],
    read: async (f) => await f.store.observationCheckpoints.getTerminalCandidate(SESSION),
    wrongShapeValue: '[1,2]',
    onInvalidJson: { kind: 'quarantine', value: null },
    onWrongShape: { kind: 'quarantine', value: null },
  },
  ...
  },
  {
    ...
    read: async (f) => await f.store.shipping.getOrder(f.stackedOrderId),
    wrongShapeValue: '[1,2]',
  ...
  },
  {
    ...
    where: 'id = ?',
    params: (f) => [f.orderId],
    read: async (f) => await f.store.shipping.getOrder(f.orderId),
    wrongShapeValue: '[1,2]',
  ...
  },
  {
    ...
    where: 'id = ?',
    params: (f) => [f.stepId],
    read: async (f) => await f.store.shipping.stepById(f.stepId),
    wrongShapeValue: '[1,2]',
  ...
  },
  {
    ...
    where: 'id = ?',
    params: (f) => [f.trainId],
    read: async (f) => await f.store.shipping.trainManifestForAttempt(f.attemptId),
    wrongShapeValue: '[1,2]',
