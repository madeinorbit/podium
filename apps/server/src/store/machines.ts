...
export class MachinesRepository {
  ...
  listMachines(): MachineRecord[] {
    return (
      this.db
        .prepare(
          'SELECT id, name, hostname, created_at, last_seen_at, inventory_json, owner_user_id, podium_managed FROM machines ORDER BY created_at ASC',
        )
  ...
  getMachine(id: string): MachineRecord | undefined {
    const r = this.db
      .prepare(
        'SELECT id, name, hostname, created_at, last_seen_at, inventory_json, owner_user_id, podium_managed FROM machines WHERE id = ?',
      )
