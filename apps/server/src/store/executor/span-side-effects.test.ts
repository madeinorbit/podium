...
async function openWiredStore(): Promise<{ store: SessionStore; ledger: Ledger }> {
...
describe('a nested ledger.commit publishes after the OUTER commit', () => {
  ...
  it('tells nobody about changes the outer body rolled back', async () => {
    const { store, ledger } = await openWiredStore()
    const delivered: string[] = []
    ledger.onAppended((changes) => {
      for (const change of changes) delivered.push(change.id)
    ...
    // in the first place.
    expect(delivered).toEqual([])
    expect(await store.sync.maxChangeSeq()).toBe(0)
  })

  it('still delivers immediately when the commit is the whole transaction', async () => {
    ...
    const { ledger } = await openWiredStore()
    const delivered: string[] = []
    ledger.onAppended((changes) => {
      for (const change of changes) delivered.push(change.id)
    })
...
describe('a subscriber that commits from inside its own notification', () => {
  it('delivers batch N to every subscriber before N+1', async () => {
...
describe('a durable follow-up that rejects', () => {
  it('never reads as a rollback: the write is committed and the error says so', async () => {
    ...
    expect(await store.sync.maxChangeSeq()).toBe(1)
  })
})
