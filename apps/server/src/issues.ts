...
export class IssueService {
   ...
   *  Seq is unique per repo_id; when several repos share a seq the caller may pass
   *  `scopeRepoPath` to narrow to its own repo (so an agent's own `#N` resolves without
   *  the full id). Still-ambiguous refs throw; unresolvable refs return the input
