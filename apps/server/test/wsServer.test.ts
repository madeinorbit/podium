...
import { connectWs, openWs, waitFor, waitMessage } from './wsTestUtil'
...
describe('server WebSocket relay', () => {
  it('relays a daemon agentFrame to a client as an epoch-stamped outputFrame', async () => {
    ...
    try {
      ...
      await waitFor(() => srv.hub.info().sessionId === 's1') // daemon bound before client joins
      const { ws: client, opened } = connectWs(`${base}/client`)
      // Register listener before open so the synchronous welcome is not missed
      const welcomeP = waitMessage(client, parseServerMessage, (m) => m.type === 'welcome')
      await opened
      try {
