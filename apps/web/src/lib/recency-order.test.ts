...
describe('compareRecency', () => {
  it('orders newest-active first', () => {
    const older = meta({ sessionId: asSessionId('a'), lastActiveAt: '2026-06-09T00:00:00.000Z' })
    const newer = meta({ sessionId: asSessionId('b'), lastActiveAt: '2026-06-10T00:00:00.000Z' })
  ...
  it('breaks ties deterministically (createdAt desc, then sessionId) — no reshuffle', () => {
    ...
    const a = meta({ sessionId: asSessionId('aaa'), createdAt: '2026-06-01T00:00:00.000Z' })
    const b = meta({ sessionId: asSessionId('bbb'), createdAt: '2026-06-02T00:00:00.000Z' })
...
describe('stable ordering with equal lastActiveAt', () => {
  it('sortSessionsForSidebar is independent of input order', () => {
    const a = meta({ sessionId: asSessionId('aaa'), createdAt: '2026-06-01T00:00:00.000Z' })
    const b = meta({ sessionId: asSessionId('bbb'), createdAt: '2026-06-02T00:00:00.000Z' })
    const c = meta({ sessionId: asSessionId('ccc'), createdAt: '2026-06-03T00:00:00.000Z' })
...
describe('partitionWorkItems (sidebar WORK ITEMS) ordering', () => {
  ...
  it('never shows shells in NEEDS YOUR ATTENTION (an idle shell is not a work item)', () => {
    const idleShell = meta({ sessionId: asSessionId('sh'), agentKind: 'shell' }) // not busy → idle shell
    const blockedAgent = agent({ sessionId: asSessionId('ag'), agentState: needsUser(recent) })
  ...
  it('shells never appear in the sidebar — not even a busy one in WORKING', () => {
    const busyShell = meta({ sessionId: asSessionId('sh'), agentKind: 'shell', busy: true })
    const {
      attention,
      working: w,
      pinnedPanels,
