...
describe('AgentPanel active wiring', () => {
  ...
  it('the initial active reflects chat mode: a panel that starts in chat mounts no terminal', async () => {
    storePanelMode = { s1: 'chat' }
    await act(async () => {
      root.render(<AgentPanel sessionId="s1" active />)
    })
    await flush()
    // Chat mode renders ChatView, not the xterm — the terminal is never mounted,
    // so it can never drive the PTY size. (effectiveMode === 'chat' short-circuits
    // the mount effect.)
    expect(mountSessionMock).not.toHaveBeenCalled()
  })

  it('stops the terminal driving size when a mounted native panel switches to chat mode', async () => {
    await act(async () => {
      root.render(<AgentPanel sessionId="s1" active />)
    })
    await flush()
    ...
    dispose.mockClear()
    // Flip the persisted mode to chat; effectiveMode becomes 'chat'. The mount
    // effect (which owns native<->chat) tears the terminal down — disposing it is
    // what stops it driving the PTY size once it's no longer the visible mode.
    storePanelMode = { s1: 'chat' }
    await act(async () => {
      root.render(<AgentPanel sessionId="s1" active />)
    })
