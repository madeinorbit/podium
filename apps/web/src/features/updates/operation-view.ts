...
function deferredNote(operation: Operation): string | undefined {
  ...
  const subject = (places: typeof deferred): string => {
    const names = places.flatMap((place) => (place.name ? [place.name] : []))
    return names.length > 0 && names.length <= 3
      ? names.join(', ')
      : `${places.length} machine${places.length === 1 ? '' : 's'}`
  }
  const offline = deferred.filter((place) => place.reason === 'offline')
  const cannot = deferred.filter((place) => place.reason !== 'offline')
  const lines: string[] = []
  if (offline.length > 0) {
    lines.push(
      `${subject(offline)} will update when ${offline.length === 1 ? 'it reconnects' : 'they reconnect'}.`,
    )
  }
  for (const place of cannot) {
    const reason =
      place.reason && place.reason !== 'cannot-take-delivery'
        ? `: ${place.reason.replaceAll('-', ' ')}`
        : ''
    lines.push(`${place.name ?? 'A machine'} cannot take this update${reason}.`)
  }
  return lines.join(' ')
}
...
function computeView(input: OperationViewInput): UpdatePanelView {
  ...
  if (operation.state === 'done') {
    ...
    return {
      ...base,
      state: 'done',
      title: hasDeliveryRefusal
        ? version
          ? `Podium ${version} update complete`
          : 'Podium update complete'
        : version
