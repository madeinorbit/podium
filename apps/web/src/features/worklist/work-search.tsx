 ...
 *
 * A 30px well between the spawn row and the list: search glyph, the field, and a
 ...
 * (spawn-row), which is gated on the native shell because a browser tab never
...
export function WorkSearchField({ filter }: { filter: WorkFilter }): JSX.Element {
  ...
  return (
    ...
    >
      <Search size={13} className="flex-none text-text-faint" aria-hidden="true" />
      <input
        ref={inputRef}
        value={query}
