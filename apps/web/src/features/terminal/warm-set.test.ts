...
describe('updateRecency', () => {
  it('moves active ids to the front, preserving order of the rest', () => {
    expect(
      updateRecency(
        [asSessionId('a'), asSessionId('b'), asSessionId('c')],
        [asSessionId('c')],
        [asSessionId('a'), asSessionId('b'), asSessionId('c')],
      ),
    ).toEqual(['c', 'a', 'b'])
  ...
  it('adds a newly-active id not seen before to the front', () => {
    expect(
      updateRecency(
        ...
        [asSessionId('d')],
        [asSessionId('a'), asSessionId('b'), asSessionId('d')],
      ),
    ).toEqual(['d', 'a', 'b'])
  })
...
describe('computeWarmSet', () => {
  it('keeps the N most-recent, always including active', () => {
    expect(
      computeWarmSet(
        [asSessionId('a'), asSessionId('b'), asSessionId('c'), asSessionId('d'), asSessionId('e')],
        [asSessionId('a')],
        3,
      ),
  ...
  it('keeps every active split pane when the measured budget is exhausted', () => {
    expect(
      computeWarmSet(
        [asSessionId('a'), asSessionId('b'), asSessionId('c'), asSessionId('d')],
        [asSessionId('a'), asSessionId('b'), asSessionId('c'), asSessionId('d')],
        3,
      ),
  ...
  it('fills remaining capacity from recency after active', () => {
    expect(
      computeWarmSet(
        [asSessionId('x'), asSessionId('a'), asSessionId('b'), asSessionId('c')],
        [asSessionId('c')],
        3,
      ),
    ).toEqual(new Set(['c', 'x', 'a']))
  })
  it('returns all when fewer than capacity', () => {
    expect(computeWarmSet([asSessionId('a'), asSessionId('b')], [asSessionId('a')], 8)).toEqual(
      new Set(['a', 'b']),
    )
  })
})
