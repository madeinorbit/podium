...
**The Reading Tier (POD-407).** The 12px base is the density of surfaces you *live* in — the shell, its columns, its rows. A surface you *visit*, read a sentence on, decide and leave gets one step up: 15px section heading, 13.5px/500 row label and field, 13px/1.6 prose on a 62ch measure, 12px micro, 9.5px mono eyebrow. Settings is the first tenant (`.settings-*` in styles.css); any future utility sheet inherits it rather than inventing its own. Two constraints keep it from becoming a second design: **chrome never joins it** — a sheet's own header, and the command bar behind it, stay at the shell's 12px, because a utility whose frame grew with its content stops reading as part of the app — and **every step moves on two axes**, size plus weight or ink, because 13 against 13.5 is not a hierarchy on its own.

**The subject step: 18px/600, −0.012em (POD-591).** A surface whose whole purpose is ONE named thing — the task detail page, and any successor that opens a single record — gets one step above the Reading Tier's section heading for that thing's name, and for nothing else on the page. It is a documented step rather than a per-page choice because the task page had been carrying an undocumented 22px: nearly twice the shell's base, taller than any heading in the design system, and enough that the title read as a banner rather than as the subject of the sheet under it. 18px against the 15px section heading and 13px prose is a legible three-rung ladder; 22px was a jump with nothing between it and the body. Tracking tightens at this size because Geist's default spacing opens up as the optical size grows. **One per surface.** A second 18px line makes the first one chrome.
