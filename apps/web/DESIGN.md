...
1. **Mode tabs.** One per tool (Work, Tasks, Workflows, Specs, Automations): a 13px glyph plus an 11.5px label at 28px height. **Only the ACTIVE tab is contained** — Chip Navy, a Border Strong ring, and the same 1px Superade Yellow inset top line the pane tabs use for "you are here". Inactive tabs are bare; the icon already carries the "this is a control" signal, and a permanently-lit brand fill is the exact spend The Signal Rule guards against.
...
3. **The instrument well.** Host and quota in ONE ringed well divided by hairlines, never loose readouts on the bar: one object with internal structure reads as an instrument, five evenly-spaced numbers read as a website's account row. Always visible, in every mode.
4. **Utilities.** Usage and Settings as 28px icon cells at the far right, after a hairline, followed by the native window controls on Windows/Linux.

**Surface:** `--bar`, the darkest tier — one step below the panels beneath it, so every column reads as carved INTO the chassis. **Narrow behaviour:** the bar gives up words before it gives up controls — tool labels drop at 1180px, mode labels at 940px, tooltips carrying the names. Nothing is ever removed.

### The Sheet Tier
Utilities are visited and left, so they open as a **bounded inset sheet over a live shell** (≤1180px, centred, gapped from the command bar and status strip), never as a route that replaces the window. The chrome stays visible around and behind them; Esc, the backdrop and the ✕ all close, returning to the mode you came from. A sheet is the one thing licensed to lift — The Carved Rule reserves drop shadows for what will disappear. Inside, panes fill the sheet and the measure caps the *text*, never the pane.
