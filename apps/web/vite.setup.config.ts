// Throwaway harness config (POD-1225) — the real vite config with the PWA
// plugin dropped and `@/app/store` aliased to a stub, so the SHIPPING
// FirstTaskActivation renders without a server behind it.
...
export default defineConfig(async (env) => {
  ...
  return {
    ...
    resolve: {
      ...resolve,
      alias: {
        '@/app/store': fileURLToPath(new URL('./harness/setup-store.ts', import.meta.url)),
        ...(resolve.alias ?? {}),
      },
    },
