...
describe('repoOpCommand', () => {
  ...
  it('cleanup ops: worktreeRemove / branchDelete / isMergedInto (never --force / -D by default)', () => {
    ...
    expect(repoOpCommand('worktreeRemove', { path: '/r/.worktrees/issue-1-x', force: '1' }))
      .toEqual({
        bin: 'git',
  ...
  it('integrate ops: worktreeAddReset / checkoutReset / checkout / rebaseAbort (issue #70)', () => {
    expect(repoOpCommand('worktreeAddReset', { path: '/r/.worktrees/integrate-9-e', branch: 'integrate/9-e', startPoint: 'main' }))
      .toEqual({ bin: 'git', argv: ['worktree', 'add', '-B', 'integrate/9-e', '--', '/r/.worktrees/integrate-9-e', 'main'] })
  ...
  it('branchDeleteForce only inside the integrate-tmp/ namespace', () => {
    ...
    expect(repoOpCommand('branchDeleteForce', { branch: '-D' }))
      .toEqual({ error: 'branchDeleteForce is restricted to integrate-tmp/* refs' })
  })
  ...
  describe('leading-dash hardening (issue #81)', () => {
    it('assertSafeRef rejects leading-dash values with a readable label', () => {
      ...
      expect(assertSafeRef('-D', 'branch')).toBe("unsafe branch: must not start with '-' (got '-D')")
      expect(assertSafeRef('--force', 'startPoint')).toBe("unsafe startPoint: must not start with '-' (got '--force')")
    })
    it('validation-guarded ops reject dash values before spawn (no -- support)', () => {
      ...
      expect(repoOpCommand('checkout', { branch: '-D' }))
        .toEqual({ error: "unsafe branch: must not start with '-' (got '-D')" })
      expect(repoOpCommand('checkoutReset', { branch: '--force', startPoint: 'main' }))
        .toEqual({ error: "unsafe branch: must not start with '-' (got '--force')" })
      expect(repoOpCommand('checkoutReset', { branch: 'integrate/9-e', startPoint: '-D' }))
        .toEqual({ error: "unsafe startPoint: must not start with '-' (got '-D')" })
      // gh pr create: flag-value slots, `--` gives no protection
      expect(repoOpCommand('prCreate', { branch: '-D', parentBranch: 'main' }))
        .toEqual({ error: "unsafe branch: must not start with '-' (got '-D')" })
      expect(repoOpCommand('prCreate', { branch: 'issue/1-x', parentBranch: '--force' }))
        .toEqual({ error: "unsafe parentBranch: must not start with '-' (got '--force')" })
      // worktree add -b/-B: the branch is an option argument `--` cannot protect
      expect(repoOpCommand('worktreeAdd', { path: '/r/wt', branch: '-D' }))
        .toEqual({ error: "unsafe branch: must not start with '-' (got '-D')" })
      expect(repoOpCommand('worktreeAddReset', { path: '/r/wt', branch: '--force', startPoint: 'main' }))
        .toEqual({ error: "unsafe branch: must not start with '-' (got '--force')" })
    })
    it('--separated ops keep dash values in ref/path slots (spawn-side literal)', () => {
      expect(repoOpCommand('rebase', { parentBranch: '-D' }))
        .toEqual({ bin: 'git', argv: ['rebase', '--', '-D'] })
      expect(repoOpCommand('mergeFfOnly', { branch: '--force' }))
        .toEqual({ bin: 'git', argv: ['merge', '--ff-only', '--', '--force'] })
      expect(repoOpCommand('branchDelete', { branch: '-D' }))
        .toEqual({ bin: 'git', argv: ['branch', '-d', '--', '-D'] })
      expect(repoOpCommand('isMergedInto', { branch: '-D', parentBranch: 'main' }))
        .toEqual({ bin: 'git', argv: ['merge-base', '--is-ancestor', '--', '-D', 'main'] })
      expect(repoOpCommand('worktreeRemove', { path: '--force' }))
        .toEqual({ bin: 'git', argv: ['worktree', 'remove', '--', '--force'] })
    })
  })
})
...
describe('repoOpCommand against a real git scratch repo (issue #81)', () => {
  ...
  const git = (argv: string[], cwd = repo): { code: number; out: string } => {
    try {
      const out = execFileSync('git', argv, { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] })
      return { code: 0, out }
    } catch (e) {
  ...
  it('happy paths still work through the -- forms', () => {
    expect(run(repoOpCommand('isMergedInto', { branch: 'victim', parentBranch: 'main' })).code).toBe(0)
    ...
    expect(run(repoOpCommand('worktreeAdd', { path: wt, branch: 'issue/1-x', startPoint: 'main' })).code).toBe(0)
